...
/**
 * This class represents an access token returned by the Facebook Login service,
 * along with associated metadata such as its expiration date and permissions.
 * In general, the {@link Session} class will abstract away the need to worry
 * about the details of an access token, but there are situations (such as
 * handling native links, importing previously-obtained access tokens, etc.)
 * where it is useful to deal with access tokens directly. Factory methods are
...
public final class AccessToken implements Serializable {
	...
	AccessToken(String token, Date expires, List<String> permissions,
			List<String> declinedPermissions, AccessTokenSource source,
			Date lastRefresh) {
		if (permissions == null) {
		...
		this.declinedPermissions = Collections
				.unmodifiableList(declinedPermissions);
		this.token = token;
		this.source = source;
	...
	/**
	 * Gets the list of permissions associated with this access token. Note that
	 * the most up-to-date list of permissions is maintained by the Facebook
	 * service, so this list may be outdated if permissions have been added or
	 * removed since the time the AccessToken object was created. For more
	 * information on permissions, see
	 * https://developers.facebook.com/docs/reference/login/#permissions.
	 *
	 * @return a read-only list of strings representing the permissions granted
	 *         via this access token
	 */
	 ...
	 * It represents the entire set of permissions that have been requested and
	 * declined. Note that the most up-to-date list of permissions is maintained
	 * by the Facebook service, so this list may be outdated if permissions have
	 * been granted or declined since the last time an AccessToken object was
	 * created.
	 *
	 * @return a read-only list of strings representing the permissions declined
	 *         by the user
	 */
	...
	/**
	 * Gets the {@link AccessTokenSource} indicating how this access token was
	 * obtained.
	 *
	...
	/**
	 * Gets the date at which the token was last refreshed. Since tokens expire,
	 * the Facebook SDK will attempt to renew them periodically.
	 *
	...
	/**
	 * Creates a new AccessToken using the supplied information from a
	 * previously-obtained access token (for instance, from an already-cached
	 * access token obtained prior to integration with the Facebook SDK).
	 *
	 * @param accessToken
	 *            the access token string obtained from Facebook
	 * @param expirationTime
	 *            the expiration date associated with the token; if null, an
	 *            infinite expiration time is assumed (but will become correct
	 ...
	 *            AccessTokenSource.WEB_VIEW); if null, FACEBOOK_APPLICATION is
	 *            assumed.
	 * @param permissions
	...
	public static AccessToken createFromExistingAccessToken(String accessToken,
			Date expirationTime, Date lastRefreshTime,
			AccessTokenSource accessTokenSource, List<String> permissions) {
		...
		return new AccessToken(accessToken, expirationTime, permissions, null,
				accessTokenSource, lastRefreshTime);
	}

	/**
	 * Creates a new AccessToken using the information contained in an Intent
	 * populated by the Facebook application in order to launch a native link.
	 * For more information on native linking, please see
	 * https://developers.facebook.com/docs/mobile/android/deep_linking/.
	 *
	 * @param intent
	 *            the Intent that was used to start an Activity; must not be
	 *            null
	 * @return a new AccessToken, or null if the Intent did not contain enough
	 *         data to create one
	 */
	public static AccessToken createFromNativeLinkingIntent(Intent intent) {
		...
		return createFromBundle(null, intent.getExtras(),
	...
	static AccessToken createEmptyToken() {
		return new AccessToken("", ALREADY_EXPIRED_EXPIRATION_TIME, null, null,
				AccessTokenSource.NONE, DEFAULT_LAST_REFRESH_TIME);
	}

	static AccessToken createFromString(String token, List<String> permissions,
			AccessTokenSource source) {
		return new AccessToken(token, DEFAULT_EXPIRATION_TIME, permissions,
				null, source, DEFAULT_LAST_REFRESH_TIME);
	}

	static AccessToken createFromNativeLogin(Bundle bundle,
			AccessTokenSource source) {
		Date expires = getBundleLongAsDate(bundle,
				NativeProtocol.EXTRA_EXPIRES_SECONDS_SINCE_EPOCH, new Date(0));
		ArrayList<String> permissions = bundle
	...
	static AccessToken createFromWebBundle(List<String> requestedPermissions,
			Bundle bundle, AccessTokenSource source) {
		Date expires = getBundleLongAsDate(bundle, EXPIRES_IN_KEY, new Date());
		String token = bundle.getString(ACCESS_TOKEN_KEY);

		// With Login v4, we now get back the actual permissions granted, so
		// update the permissions to be the real thing
		...
		return createNew(requestedPermissions, declinedPermissions, token,
				expires, source);
	}
	...
	static AccessToken createFromRefresh(AccessToken current, Bundle bundle) {
		...
		return createNew(current.getPermissions(),
				current.getDeclinedPermissions(), token, expires,
				current.source);
	}

	static AccessToken createFromTokenWithRefreshedPermissions(
			AccessToken token, List<String> grantedPermissions,
			List<String> declinedPermissions) {
		return new AccessToken(token.token, token.expires, grantedPermissions,
				declinedPermissions, token.source, token.lastRefresh);
	}

	private static AccessToken createNew(List<String> grantedPermissions,
			List<String> declinedPermissions, String accessToken, Date expires,
			AccessTokenSource source) {
		if (Utility.isNullOrEmpty(accessToken) || (expires == null)) {
			return createEmptyToken();
		} else {
			return new AccessToken(accessToken, expires, grantedPermissions,
					declinedPermissions, source, new Date());
		}
	}

	static AccessToken createFromCache(Bundle bundle) {
		List<String> permissions = getPermissionsFromBundle(bundle,
				TokenCachingStrategy.PERMISSIONS_KEY);
		List<String> declinedPermissions = getPermissionsFromBundle(bundle,
	...
	static List<String> getPermissionsFromBundle(Bundle bundle, String key) {
		...
		if (originalPermissions == null) {
			...
			permissions = Collections.unmodifiableList(new ArrayList<String>(
					originalPermissions));
		}
		return permissions;
	}
	...
	Bundle toCacheBundle() {
		...
		TokenCachingStrategy.putDate(bundle,
				TokenCachingStrategy.EXPIRATION_DATE_KEY, expires);
		...
		TokenCachingStrategy.putDate(bundle,
				TokenCachingStrategy.LAST_REFRESH_DATE_KEY, lastRefresh);

		return bundle;
	}

	boolean isInvalid() {
		return Utility.isNullOrEmpty(this.token)
				|| new Date().after(this.expires);
	}

	private static AccessToken createFromBundle(
			List<String> requestedPermissions, Bundle bundle,
			AccessTokenSource source, Date expirationBase) {
		String token = bundle.getString(ACCESS_TOKEN_KEY);
		Date expires = getBundleLongAsDate(bundle, EXPIRES_IN_KEY,
				expirationBase);
		...
		return new AccessToken(token, expires, requestedPermissions, null,
				source, new Date());
	}
	...
	private String tokenToString() {
		if (this.token == null) {
			return "null";
				...
				.isLoggingBehaviorEnabled(LoggingBehavior.INCLUDE_ACCESS_TOKENS)) {
			return this.token;
		} else {
	...
	private static class SerializationProxyV2 implements Serializable {
		...
		private Object readResolve() {
			return new AccessToken(token, expires, permissions,
					declinedPermissions, source, lastRefresh);
		}
	}

	private Object writeReplace() {
		return new SerializationProxyV2(token, expires, permissions,
				declinedPermissions, source, lastRefresh);
	}
	...
	private static Date getBundleLongAsDate(Bundle bundle, String key,
			Date dateBase) {
		if (bundle == null) {
			return null;
		}
