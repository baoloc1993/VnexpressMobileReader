...
/**
 * <p>
 * The AppEventsLogger class allows the developer to log various types of events
 * back to Facebook. In order to log events, the app must create an instance of
 * this class via a {@link #newLogger newLogger} method, and then call the
 * various "log" methods off of that.
 * </p>
 * <p>
 * This client-side event logging is then available through Facebook App
 * Insights and for use with Facebook Ads conversion tracking and optimization.
 * </p>
 * <p>
 * The AppEventsLogger class has a few related roles:
 * <ul>
 * <li>
 * Logging predefined and application-defined events to Facebook App Insights
 * with a numeric value to sum across a large number of events, and an optional
 * set of key/value parameters that define "segments" for this event (e.g.,
 * 'purchaserStatus' : 'frequent', or 'gamerLevel' : 'intermediate'). These
 * events may also be used for ads conversion tracking, optimization, and other
 * ads related targeting in the future.</li>
 * <li>
 * Methods that control the way in which events are flushed out to the Facebook
 * servers.</li>
 * </ul>
 ...
 * <ul>
 * <li>
 * Events are not sent immediately when logged. They're cached and flushed out
 * to the Facebook servers in a number of situations:
 * <ul>
 ...
 * </ul>
 * <li>
 * Events will be accumulated when the app is in a disconnected state, and sent
 * when the connection is restored and one of the above 'flush' conditions are
 * met.</li>
 * <li>
 * The AppEventsLogger class is intended to be used from the thread it was
 * created on. Multiple AppEventsLoggers may be created on other threads if
 * desired.</li>
 * <li>
 * The developer can call the setFlushBehavior method to force the flushing of
 * events to only occur on an explicit call to the `flush` method.</li>
 * <li>
 * The developer can turn on console debug output for event logging and flushing
 * to the server Settings.addLoggingBehavior(LoggingBehavior.APP_EVENTS);</li>
 * </ul>
 * Some things to note when logging events:
 * <ul>
 * <li>
 * There is a limit on the number of unique event names an app can use, on the
 * order of 300.</li>
 * <li>
 * There is a limit to the number of unique parameter names in the provided
 * parameters that can be used per event, on the order of 25. This is not just
 * for an individual call, but for all invocations for that eventName.</li>
 * <li>
 * Event names and parameter names (the keys in the NSDictionary) must be
 * between 2 and 40 characters, and must consist of alphanumeric characters, _,
 * -, or spaces.</li>
 * <li>
 * The length of each parameter value can be no more than on the order of 100
 * characters.</li>
 * </ul>
 */
public class AppEventsLogger {
	...
	public enum FlushBehavior {
		/**
		 * Flush automatically: periodically (once a minute or after every 100
		 * events), and always at app reactivation. This is the default value.
		 */
		AUTO,
	...
	private enum SuppressionTimeoutBehavior {
		// Successfully logging an event will reset the timeout period (i.e.,
		// events will log no more than every N
		// seconds).
		RESET_TIMEOUT_WHEN_LOG_SUCCESSFUL,
		// Attempting to log an event, even if it is suppressed, will reset the
		// timeout period (i.e., events will not
	...
	private static Map<String, EventSuppression> mapEventNameToSuppress = new HashMap<String, EventSuppression>() {
		{
			put(AppEventsConstants.EVENT_NAME_ACTIVATED_APP,
					new EventSuppression(
							APP_ACTIVATE_SUPPRESSION_PERIOD_IN_SECONDS,
							SuppressionTimeoutBehavior.RESET_TIMEOUT_WHEN_LOG_ATTEMPTED));
		}
	};

	// Rather than retaining Sessions, we extract the information we need and
	// track app events by
	// application ID and access token (which may be null for Session-less
	// calls). This avoids needing to
	// worry about Session lifecycle and also allows us to coalesce app events
	...
	private static class AccessTokenAppIdPair implements Serializable {
		...
		AccessTokenAppIdPair(String accessToken, String applicationId) {
			this.accessToken = Utility.isNullOrEmpty(accessToken) ? null
					: accessToken;
	 ...
	 * {@link Settings#getLimitEventAndDataUsage(Context)} instead.
	 */
	@Deprecated
	...
	/**
	 * This method is deprecated. Use
	 * {@link Settings#setLimitEventAndDataUsage(Context, boolean)} instead.
	 */
	@Deprecated
	 ...
	 *
	 * @param context
	 *            Used to access the applicationId and the attributionId for
	...
	public static void activateApp(Context context, String applicationId) {
		...
		// activateApp supercedes publishInstall in the public API, so we need
		// to explicitly invoke it, since the server
		// can't reliably infer install state for all conditions of an app
		// activate.
		Settings.publishInstallAsync(context, applicationId);

		AppEventsLogger logger = new AppEventsLogger(context, applicationId,
				null);
		logger.logEvent(AppEventsConstants.EVENT_NAME_ACTIVATED_APP);
	}

	/**
	 * Build an AppEventsLogger instance to log events through. The Facebook app
	 * that these events are targeted at comes from this application's metadata.
	 * The application ID used to log events will be determined from the app ID
	 ...
	 *            Used to access the attributionId for non-authenticated users.
	 * @param session
	 *            Explicitly specified Session to log events against. If null,
	 *            the activeSession will be used if it's open, otherwise the
	 *            logging will happen against the default app ID specified via
	 ...
	 *            Used to access the attributionId for non-authenticated users.
	 * @param applicationId
	 *            Explicitly specified Facebook applicationId to log events
	 *            against. If null, the default app ID specified in the package
	 *            metadata will be used.
	 * @param session
	 *            Explicitly specified Session to log events against. If null,
	 *            the activeSession will be used if it's open, otherwise the
	 *            logging will happen against the specified app ID.
	 *
	 * @return AppEventsLogger instance to invoke log* methods on.
	 */
	public static AppEventsLogger newLogger(Context context,
	...
	/**
	 * Build an AppEventsLogger instance to log events that are attributed to
	 * the application but not to any particular Session.
	 *
	 * @param context
	 *            Used to access the attributionId for non-authenticated users.
	 * @param applicationId
	...
	/**
	 * The action used to indicate that a flush of app events has occurred. This
	...
	/**
	 * Access the behavior that AppEventsLogger uses to determine when to flush
	 * logged events to the server. This setting applies to all instances of
	 * AppEventsLogger.
	 *
	...
	/**
	 * Set the behavior that this AppEventsLogger uses to determine when to
	 * flush logged events to the server. This setting applies to all instances
	 * of AppEventsLogger.
	 *
	 * @param flushBehavior
	 ...
	 *
	 * @param eventName
	 *            eventName used to denote the event. Choose amongst the
	 *            EVENT_NAME_* constants in {@link AppEventsConstants} when
	 *            possible. Or create your own if none of the EVENT_NAME_*
	 *            constants are applicable. Event names should be 40 characters
	 *            or less, alphanumeric, and can include spaces, underscores or
	 *            hyphens, but mustn't have a space or hyphen as the first
	 *            character. Any given app should have no more than ~300
	 ...
	 *            hyphens, but mustn't have a space or hyphen as the first
	 *            character. Any given app should have no more than ~300
	 *            distinct event names. * @param eventName
	 * @param valueToSum
	 *            a value to associate with the event which will be summed up in
	 *            Insights for across all instances of the event, so that
	 *            average values can be determined, etc.
	 */
	 ...
	 *            distinct event names.
	 * @param parameters
	 *            A Bundle of parameters to log with the event. Insights will
	 *            allow looking at the logs of these events via different
	 *            parameter values. You can log on the order of 10 parameters
	 *            with each distinct eventName. It's advisable to keep the
	 *            number of unique values provided for each parameter in the, at
	 *            most, thousands. As an example, don't attempt to provide a
	 *            unique parameter value for each unique user in your app. You
	 *            won't get meaningful aggregate reporting on so many parameter
	 *            values. The values in the bundles should be Strings or numeric
	 *            values.
	 */
	...
	/**
	 * Log an app event with the specified name, supplied value, and set of
	 * parameters.
	 *
	 * @param eventName
	 *            eventName used to denote the event. Choose amongst the
	...
	/**
	 * Logs a purchase event with Facebook, in the specified amount and with the
	 * specified currency.
	 *
	 * @param purchaseAmount
	 *            Amount of purchase, in the currency specified by the
	 *            'currency' parameter. This value will be rounded to the
	 *            thousandths place (e.g., 12.34567 becomes 12.346).
	 * @param currency
	 *            Currency used to specify the amount.
	 */
	...
	/**
	 * Logs a purchase event with Facebook, in the specified amount and with the
	 * specified currency. Additional detail about the purchase can be passed in
	 ...
	 *            Currency used to specify the amount.
	 * @param parameters
	 *            Arbitrary additional information for describing this event.
	 *            Should have no more than 10 entries, and keys should be mostly
	 *            consistent from one purchase event to the next.
	 */
	public void logPurchase(BigDecimal purchaseAmount, Currency currency,
			Bundle parameters) {
		...
		parameters.putString(AppEventsConstants.EVENT_PARAM_CURRENCY,
				currency.getCurrencyCode());

		logEvent(AppEventsConstants.EVENT_NAME_PURCHASED,
				purchaseAmount.doubleValue(), parameters);
		eagerFlush();
	}

	/**
	 * Explicitly flush any stored events to the server. Implicit flushes may
	 * happen depending on the value of getFlushBehavior. This method allows for
	 * explicit, app invoked flushing.
	 */
	public void flush() {
	...
	/**
	 * Call this when the consuming Activity/Fragment receives an onStop()
	 * callback in order to persist any outstanding events to disk, so they may
	 * be flushed at a later time. The next flush (explicit or not) will check
	 * for any outstanding events and, if present, include them in that flush.
	 * Note that this call may trigger an I/O operation on the calling thread.
	 * Explicit use of this method is not necessary if the consumer is making
	 * use of {@link UiLifecycleHelper}, which will take care of making the call
	...
	/**
	 * This method is intended only for internal use by the Facebook SDK and
	 * other use is unsupported.
	 */
	public void logSdkEvent(String eventName, Double valueToSum,
			Bundle parameters) {
	...
	/**
	 * Constructor is private, newLogger() methods should be used to build an
	 * instance.
	 */
	private AppEventsLogger(Context context, String applicationId,
			Session session) {
		...
		synchronized (staticLock) {
			...
			if (hashedDeviceAndAppId == null) {
				hashedDeviceAndAppId = Utility.getHashedDeviceAndAppID(context,
						applicationId);
			}
	...
	private static void initializeTimersIfNeeded() {
		...
		supportsAttributionRecheckTimer.schedule(new TimerTask() {
			...
			public void run() {
				...
				synchronized (staticLock) {
					for (AccessTokenAppIdPair accessTokenAppId : stateMap
							.keySet()) {
	...
	private void logEvent(String eventName, Double valueToSum,
			Bundle parameters, boolean isImplicitlyLogged) {

		AppEvent event = new AppEvent(eventName, valueToSum, parameters,
				isImplicitlyLogged);
		logEvent(context, event, accessTokenAppId);
	}

	private static void logEvent(Context context, AppEvent event,
			AccessTokenAppIdPair accessTokenAppId) {
		if (shouldSuppressEvent(event)) {
			return;
		}

		SessionEventsState state = getSessionEventsState(context,
				accessTokenAppId);
	...
	private static boolean shouldSuppressEvent(AppEvent event) {
		EventSuppression suppressionInfo = mapEventNameToSuppress.get(event
				.getName());
		...
		// Update the time if we're not suppressed, OR if we are suppressed but
		// the behavior is to reset even on
		// suppressed events.
		if (!suppressed
	...
	private static SessionEventsState getSessionEventsState(Context context,
			AccessTokenAppIdPair accessTokenAppId) {
		synchronized (staticLock) {
			SessionEventsState state = stateMap.get(accessTokenAppId);
			if (state == null) {
				// Retrieve attributionId, but we will only send it if
				// attribution is supported for the app.
				AttributionIdentifiers attributionIdentifiers = AttributionIdentifiers
						.getAttributionIdentifiers(context);

				state = new SessionEventsState(attributionIdentifiers,
						context.getPackageName(), hashedDeviceAndAppId);
	...
	private static void flushAndWait(final FlushReason reason) {
		...
		if (flushResults != null) {
			...
			intent.putExtra(APP_EVENTS_EXTRA_NUM_EVENTS_FLUSHED,
	...
	private static FlushStatistics buildAndExecuteRequests(FlushReason reason,
			Set<AccessTokenAppIdPair> keysToFlush) {
		FlushStatistics flushResults = new FlushStatistics();

		boolean limitEventUsage = Settings
				.getLimitEventAndDataUsage(applicationContext);
		...
		for (AccessTokenAppIdPair accessTokenAppId : keysToFlush) {
			...
			Request request = buildRequestForSession(accessTokenAppId,
					sessionEventsState, limitEventUsage, flushResults);
			if (request != null) {
		...
		if (requestsToExecute.size() > 0) {
			Logger.log(LoggingBehavior.APP_EVENTS, TAG,
					"Flushing %d events due to %s.", flushResults.numEvents,
					reason.toString());

			for (Request request : requestsToExecute) {
				// Execute the request synchronously. Callbacks will take care
				// of handling errors and updating
	...
	private static Request buildRequestForSession(
			final AccessTokenAppIdPair accessTokenAppId,
			final SessionEventsState sessionEventsState,
			final boolean limitEventUsage, final FlushStatistics flushState) {
		String applicationId = accessTokenAppId.getApplicationId();

		Utility.FetchedAppSettings fetchedAppSettings = Utility
				.queryAppSettings(applicationId, false);

		final Request postRequest = Request.newPostRequest(null,
				String.format("%s/activities", applicationId), null, null);

		Bundle requestParameters = postRequest.getParameters();
		if (requestParameters == null) {
			requestParameters = new Bundle();
		}
		requestParameters.putString("access_token",
				accessTokenAppId.getAccessToken());
		...
		postRequest.setCallback(new Request.Callback() {
			...
			public void onCompleted(Response response) {
				handleResponse(accessTokenAppId, postRequest, response,
						sessionEventsState, flushState);
			}
		});
	...
	private static void handleResponse(AccessTokenAppIdPair accessTokenAppId,
		...
		if (error != null) {
			...
			if (error.getErrorCode() == NO_CONNECTIVITY_ERROR_CODE) {
				...
				resultDescription = String.format(
						"Failed:\n  Response: %s\n  Error %s",
						response.toString(), error.toString());
		...
		if (Settings.isLoggingBehaviorEnabled(LoggingBehavior.APP_EVENTS)) {
			...
			Logger.log(
					...
					TAG,
					"Flush completed\nParams: %s\n  Result: %s\n  Events JSON: %s",
					request.getGraphObject().toString(), resultDescription,
					prettyPrintedEvents);
		}
		...
		if (flushResult == FlushResult.NO_CONNECTIVITY) {
			// We may call this for multiple requests in a batch, which is
			// slightly inefficient since in principle
			// we could call it once for all failed requests, but the impact is
			// likely to be minimal.
			// We don't call this for other server errors, because if an event
			// failed because it was malformed, etc.,
			// continually retrying it will cause subsequent events to not be
			// logged either.
			PersistedEvents.persistEvents(applicationContext, accessTokenAppId,
					sessionEventsState);
		}

		if (flushResult != FlushResult.SUCCESS) {
			// We assume that connectivity issues are more significant to report
			// than server issues.
	...
	private static int accumulatePersistedEvents() {
		PersistedEvents persistedEvents = PersistedEvents
				.readAndClearStore(applicationContext);

		int result = 0;
		for (AccessTokenAppIdPair accessTokenAppId : persistedEvents.keySet()) {
			SessionEventsState sessionEventsState = getSessionEventsState(
					applicationContext, accessTokenAppId);
	...
	/**
	 * Invoke this method, rather than throwing an Exception, for situations
	 * where user/server input might reasonably cause this to occur, and thus
	 * don't want an exception thrown at production time, but do want logging
	 * notification.
	 */
	...
	static class SessionEventsState {
		...
		public SessionEventsState(AttributionIdentifiers identifiers,
				String packageName, String hashedDeviceAndAppId) {
			this.attributionIdentifiers = identifiers;
			this.packageName = packageName;
			this.hashedDeviceAndAppId = hashedDeviceAndAppId;
		}

		// Synchronize here and in other methods on this class, because could be
		...
		public int populateRequest(Request request,
				boolean includeImplicitEvents, boolean includeAttribution,
				boolean limitEventUsage) {
			...
			populateRequest(request, numSkipped, jsonArray, includeAttribution,
					limitEventUsage);
			return jsonArray.length();
		}

		public synchronized List<AppEvent> getEventsToPersist() {
			// We will only persist accumulated events, not ones currently
			// in-flight. This means if an in-flight
			// request fails, those requests will not be persisted and thus
			// might be lost if the process terminates
		...
		public synchronized void accumulatePersistedEvents(List<AppEvent> events) {
			// We won't skip events due to a full buffer, since we already
			// accumulated them once and persisted
			// them. But they will count against the buffer size when further
			// events are accumulated.
			accumulatedEvents.addAll(events);
		}

		private void populateRequest(Request request, int numSkipped,
				JSONArray events, boolean includeAttribution,
				boolean limitEventUsage) {
			...
			if (includeAttribution) {
				Utility.setAppEventAttributionParameters(publishParams,
						attributionIdentifiers, hashedDeviceAndAppId,
						limitEventUsage);
			}
			...
			if (jsonString != null) {
				requestParameters.putByteArray("custom_events_file",
	...
	static class AppEvent implements Serializable {
		...
		public AppEvent(String eventName, Double valueToSum, Bundle parameters,
			...
			try {
				...
				// If any of the above failed, just consider this an illegal
				// event.
				Logger.log(LoggingBehavior.APP_EVENTS, "AppEvents",
		...
		private AppEvent(String jsonString, boolean isImplicit)
		...
		private void validateIdentifier(String identifier) {
			...
			// Identifier should be 40 chars or less, and only have 0-9A-Za-z,
			// underscore, hyphen, and space (but no
		...
		public String toString() {
			return String.format("\"%s\", implicit: %b, json: %s",
					jsonObject.optString("_eventName"), isImplicit,
					jsonObject.toString());
		}
	}

	// Read/write operations are thread-safe/atomic across all instances of
	// PersistedEvents, but modifications
	...
	static class PersistedEvents {
		...
		private HashMap<AccessTokenAppIdPair, List<AppEvent>> persistedEvents = new HashMap<AccessTokenAppIdPair, List<AppEvent>>();
		...
		public static void persistEvents(Context context,
			...
			synchronized (staticLock) {
				// Note that we don't track which instance of AppEventsLogger
				// added a particular event to
				// SessionEventsState; when a particular Context is being
				// destroyed, we'll persist all accumulated
				// events. More sophisticated tracking could be done to try to
				// reduce unnecessary persisting of events,
				// but the overall number of events is not expected to be large.
				PersistedEvents persistedEvents = readAndClearStore(context);

				for (Map.Entry<AccessTokenAppIdPair, SessionEventsState> entry : eventsToPersist
						.entrySet()) {
					List<AppEvent> events = entry.getValue()
							.getEventsToPersist();
					if (events.size() == 0) {
						continue;
					}
		...
		private void readAndClearStore() {
			...
			try {
				...
				// events in the case of exception rather than potentially log
				// them twice.
		...
		public void addEvents(AccessTokenAppIdPair accessTokenAppId,
