...
/**
 * GraphObject is the primary interface used by the Facebook SDK for Android to
 * represent objects in the Facebook Social Graph and the Facebook Open Graph
 * (OG). It is the base interface for all typed access to graph objects in the
 * SDK. No concrete classes implement GraphObject or its derived interfaces.
 * Rather, they are implemented as proxies (see the
 * {@link com.facebook.model.GraphObject.Factory Factory} class) that provide
 * strongly-typed property getters and setters to access the underlying data.
 * Since the primary use case for graph objects is sending and receiving them
 * over the wire to/from Facebook services, they are represented as JSONObjects.
 * No validation is done that a graph object is actually of a specific type --
 * any graph object can be treated as any GraphObject-derived interface, and the
 * presence or absence of specific properties determines its suitability for use
 * as that particular type of object. <br/>
 */
public interface GraphObject {
	/**
	 * Returns a new proxy that treats this graph object as a different
	 * GraphObject-derived type.
	 *
	 * @param graphObjectClass
	 *            the type of GraphObject to return
	 * @return a new instance of the GraphObject-derived-type that references
	 *         the same underlying data
	 */
	<T extends GraphObject> T cast(Class<T> graphObjectClass);

	/**
	 * Returns a Java Collections map of names and properties. Modifying the
	 * returned map modifies the inner JSON representation.
	 *
	...
	/**
	 * Gets a property of the GraphObject, cast to a particular
	 * GraphObject-derived interface. This gives some of the benefits of having
	 * a property getter defined to return a GraphObject-derived type without
	 ...
	 * @return
	 */
	<T extends GraphObject> T getPropertyAs(String propertyName,
			Class<T> graphObjectClass);

	/**
	 * Gets a property of the GraphObject, cast to a a list of instances of a
	 * particular GraphObject-derived interface. This gives some of the benefits
	 * of having a property getter defined to return a GraphObject-derived type
	 * without requiring explicit definition of an interface to define the
	 * getter.
	 *
	 * @param propertyName
	 *            the name of the property to get
	 * @param graphObjectClass
	 *            the GraphObject-derived interface to cast the property to a
	 *            list of
	 * @return
	 */
	<T extends GraphObject> GraphObjectList<T> getPropertyAsList(
	...
	/**
	 * Creates proxies that implement GraphObject, GraphObjectList, and their
	 * derived types. These proxies allow access to underlying collections and
	 * name/value property bags via strongly-typed property getters and setters.
	 * <p/>
	 * This supports get/set properties that use primitive types, JSON types,
	 * Date, other GraphObject types, Iterable, Collection, List, and
	 * GraphObjectList.
	 */
	final class Factory {
		...
		/**
		 * Creates a GraphObject proxy that provides typed access to the data in
		 * an underlying JSONObject.
		 *
		 * @param json
		...
		/**
		 * Creates a GraphObject-derived proxy that provides typed access to the
		...
		/**
		 * Determines if two GraphObjects represent the same underlying graph
		 * object, based on their IDs.
		 *
		 * @param a
		 *            a graph object
		 * @param b
		 *            another graph object
		 * @return true if both graph objects have an ID and it is the same ID,
		 *         false otherwise
		 */
		public static boolean hasSameId(GraphObject a, GraphObject b) {
			if (a == null || b == null || !a.asMap().containsKey("id")
					|| !b.asMap().containsKey("id")) {
				return false;
			}
			if (a.equals(b)) {
				return true;
			...
			if (idA == null || idB == null || !(idA instanceof String)
		...
		/**
		 * Creates a GraphObjectList-derived proxy that provides typed access to
		 * the data in an underlying JSONArray.
		 *
		 * @param array
		 *            the JSONArray containing the data to be exposed
		 * @param graphObjectClass
		 *            the GraphObject-derived type to return
		 * @return a graphObjectClass that represents the underlying data
		 *
		...
		/**
		 * Creates a GraphObjectList-derived proxy that initially contains no
		 * data.
		 *
		 * @param graphObjectClass
		 *            the GraphObject-derived type to return
		 ...
		 *             interface
		 */
		public static <T> GraphObjectList<T> createList(
				Class<T> graphObjectClass) {
			return createList(new JSONArray(), graphObjectClass);
		}

		private static <T extends GraphObject> T createGraphObjectProxy(
				Class<T> graphObjectClass, JSONObject state) {
			...
			T graphObject = (T) Proxy.newProxyInstance(
					GraphObject.class.getClassLoader(), interfaces,
					graphObjectProxy);

			return graphObject;
		}

		private static Map<String, Object> createGraphObjectProxyForMap(
				JSONObject state) {
		...
		private static synchronized <T extends GraphObject> boolean hasClassBeenVerified(
				Class<T> graphObjectClass) {
		...
		private static <T extends GraphObject> void verifyCanProxyClass(
			...
			if (!graphObjectClass.isInterface()) {
				throw new FacebookGraphObjectException(
						"Factory can only wrap interfaces, not class: "
			...
			for (Method method : methods) {
				...
				boolean hasPropertyNameOverride = method
						.isAnnotationPresent(PropertyName.class);

				if (method.getDeclaringClass().isAssignableFrom(
						GraphObject.class)) {
					// Don't worry about any methods from GraphObject or one of
					// its base classes.
					continue;
				} else if (parameterCount == 1 && returnType == Void.TYPE) {
					if (hasPropertyNameOverride) {
						// If a property override is present, it MUST be valid.
						// We don't fallback
						// to using the method name
						if (!Utility.isNullOrEmpty(method.getAnnotation(
								PropertyName.class).value())) {
							continue;
						}
				...
				throw new FacebookGraphObjectException(
						"Factory can't proxy method: " + method.toString());
			}

			recordClassHasBeenVerified(graphObjectClass);
		}

		// If expectedType is a generic type, expectedTypeAsParameterizedType
		// must be provided in order to determine
		// generic parameter types.
		static <U> U coerceValueToExpectedType(Object value,
			...
			if (expectedType.isPrimitive()) {
				// If the result is a primitive, let the runtime succeed or fail
				// at unboxing it.
				@SuppressWarnings("unchecked")
				U result = (U) value;
				return result;
			...
			if (GraphObject.class.isAssignableFrom(expectedType)) {
				...
				if (JSONObject.class.isAssignableFrom(valueType)) {
					...
					@SuppressWarnings("unchecked")
					U result = (U) createGraphObjectProxy(graphObjectClass,
							(JSONObject) value);
					return result;
				} else if (GraphObject.class.isAssignableFrom(valueType)) {
					// We can cast a GraphObject-derived class to another
					// GraphObject-derived class.
					@SuppressWarnings("unchecked")
					U result = (U) ((GraphObject) value).cast(graphObjectClass);
					return result;
				} else {
					throw new FacebookGraphObjectException(
							"Can't create GraphObject from "
									+ valueType.getName());
				}
			} else if (Iterable.class.equals(expectedType)
					|| Collection.class.equals(expectedType)
					|| List.class.equals(expectedType)
					|| GraphObjectList.class.equals(expectedType)) {
				if (expectedTypeAsParameterizedType == null) {
					throw new FacebookGraphObjectException(
							"can't infer generic type of: "
									+ expectedType.toString());
				}

				Type[] actualTypeArguments = expectedTypeAsParameterizedType
				...
				if (JSONArray.class.isAssignableFrom(valueType)) {
					...
					@SuppressWarnings("unchecked")
					U result = (U) createList(jsonArray,
							collectionGenericArgument);
					return result;
				} else {
					throw new FacebookGraphObjectException(
							"Can't create Collection from "
									+ valueType.getName());
				}
			...
			throw new FacebookGraphObjectException("Can't convert type"
					+ valueType.getName() + " to " + expectedType.getName());
		}
		...
		InvocationHandler {
			...
			protected final Object throwUnexpectedMethodSignature(Method method) {
				throw new FacebookGraphObjectException(getClass().getName()
						+ " got an unexpected method signature: "
						+ method.toString());
			}

			protected final Object proxyObjectMethods(Object proxy,
		...
		ProxyBase<JSONObject> {
			...
			public String toString() {
				return String.format(
						"GraphObject{graphObjectClass=%s, state=%s}",
			...
			private final Object proxyMapMethods(Method method, Object[] args) {
				...
				if (methodName.equals(CLEAR_METHOD)) {
					...
					return JsonUtil
							.jsonObjectContainsValue(this.state, args[0]);
			...
			private final Object proxyGraphObjectMethods(Object proxy,
				...
				if (methodName.equals(CAST_METHOD)) {
					...
					return coerceValueToExpectedType(value,
							GraphObjectList.class, parameterizedType);
			...
			private Object createGraphObjectsFromParameters(
					CreateGraphObject createGraphObject, Object value) {
				if (createGraphObject != null
					...
					if (List.class.isAssignableFrom(value.getClass())) {
						GraphObjectList<GraphObject> graphObjects = GraphObject.Factory
								.createList(GraphObject.class);
						@SuppressWarnings("unchecked")
						List<Object> values = (List<Object>) value;
						for (Object obj : values) {
							GraphObject graphObject = GraphObject.Factory
									.create();
			...
			private final Object proxyGraphObjectGettersAndSetters(
					Method method, Object[] args) throws JSONException {
				String methodName = method.getName();
				int parameterCount = method.getParameterTypes().length;
				PropertyName propertyNameOverride = method
						.getAnnotation(PropertyName.class);

				String key = propertyNameOverride != null ? propertyNameOverride
						.value()
		...
		AbstractList<T> implements GraphObjectList<T> {
			...
			@Override
			public String toString() {
				return String.format("GraphObjectList{itemType=%s, state=%s}",
						itemType.getSimpleName(), state);
			}

			@Override
			public void add(int location, T object) {
				// We only support adding at the end of the list, due to
				// JSONArray restrictions.
				if (location < 0) {
					throw new IndexOutOfBoundsException();
				} else if (location < size()) {
					throw new UnsupportedOperationException(
							"Only adding items at the end of the list is supported.");
				}
			...
			@Override
			public final <U extends GraphObject> GraphObjectList<U> castToListOf(
					Class<U> graphObjectClass) {
				if (GraphObject.class.isAssignableFrom(itemType)) {
					...
					throw new FacebookGraphObjectException(
							"Can't cast GraphObjectCollection of non-GraphObject type "
									+ itemType);
				}
			}
