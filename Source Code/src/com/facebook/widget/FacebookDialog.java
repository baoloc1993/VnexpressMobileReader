...
public class FacebookDialog {
	...
	public interface Callback {
		 ...
		 *
		 * @param pendingCall
		 *            a PendingCall containing the call ID and original Intent
		 *            used to launch the dialog
		 * @param data
		 *            a Bundle containing the results of the dialog, whose
		 *            contents will vary depending on the type of dialog being
		 *            displayed.
		 */
	...
	/**
	 * Defines a set of features that may be supported by the native Share
	 * dialog exposed by the Facebook application. As additional features are
	 * added, these flags may be passed to
	 * {@link FacebookDialog#canPresentShareDialog(android.content.Context, com.facebook.widget.FacebookDialog.ShareDialogFeature...)}
	 * to determine whether the version of the Facebook application installed on
	 * the user's device is recent enough to support specific features, which in
	 * turn may be used to determine which UI, etc., to present to the user.
	 */
	public enum ShareDialogFeature implements DialogFeature {
		/**
		 * Indicates whether the native Share dialog itself is supported by the
		 * installed version of the Facebook application.
		 */
		SHARE_DIALOG(NativeProtocol.PROTOCOL_VERSION_20130618),
		/**
		 * Indicates whether the native Share dialog supports sharing of photo
		 * images.
		 */
	...
	/**
	 * Defines a set of features that may be supported by the native Message
	 * dialog exposed by the Facebook Messenger application. As additional
	 * features are added, these flags may be passed to
	 * {@link FacebookDialog#canPresentMessageDialog(android.content.Context, com.facebook.widget.FacebookDialog.MessageDialogFeature...)}
	 * to determine whether the version of the Facebook application installed on
	 * the user's device is recent enough to support specific features, which in
	 * turn may be used to determine which UI, etc., to present to the user.
	 */
	public enum MessageDialogFeature implements DialogFeature {
		/**
		 * Indicates whether the native Message dialog itself is supported by
		 * the installed version of the Facebook application.
		 */
		MESSAGE_DIALOG(NativeProtocol.PROTOCOL_VERSION_20140204),
		/**
		 * Indicates whether the native Message dialog supports sharing of photo
		 * images.
		 */
	...
	/**
	 * Defines a set of features that may be supported by the native Open Graph
	 * dialogs exposed by the Facebook application. As additional features are
	 * added, these flags may be passed to
	 * {@link FacebookDialog#canPresentOpenGraphActionDialog(android.content.Context, com.facebook.widget.FacebookDialog.OpenGraphActionDialogFeature...)}
	 * to determine whether the version of the Facebook application installed on
	 * the user's device is recent enough to support specific features, which in
	 * turn may be used to determine which UI, etc., to present to the user.
	 */
	public enum OpenGraphActionDialogFeature implements DialogFeature {
		/**
		 * Indicates whether the native Open Graph action dialog itself is
		 * supported by the installed version of the Facebook application.
		 */
	...
	/**
	 * Defines a set of features that may be supported by the native Open Graph
	 * Message dialogs exposed by the Facebook application. As additional
	 * features are added, these flags may be passed to
	 * {@link FacebookDialog#canPresentOpenGraphMessageDialog(android.content.Context, com.facebook.widget.FacebookDialog.OpenGraphMessageDialogFeature...)}
	 * to determine whether the version of the Facebook application installed on
	 * the user's device is recent enough to support specific features, which in
	 * turn may be used to determine which UI, etc., to present to the user.
	 */
	public enum OpenGraphMessageDialogFeature implements DialogFeature {
		/**
		 * Indicates whether the native Open Graph Message dialog itself is
		 * supported by the installed version of the Messenger application.
		 */
	...
	private FacebookDialog(Activity activity, Fragment fragment,
			PendingCall appCall, OnPresentCallback onPresentCallback) {
	...
	public PendingCall present() {
		logDialogActivity(activity, fragment,
				getEventName(appCall.getRequestIntent()),
		...
		if (fragment != null) {
			fragment.startActivityForResult(appCall.getRequestIntent(),
					appCall.getRequestCode());
		} else {
			activity.startActivityForResult(appCall.getRequestIntent(),
					appCall.getRequestCode());
		}
		return appCall;
	}
	 ...
	 *
	 * @param context
	 *            the Context that is handling the activity result
	 * @param appCall
	 *            an PendingCall containing the call ID and original Intent used
	 ...
	 *            the result Intent
	 * @param callback
	 *            a callback to call after parsing the results
	 * @return true if the activity result was handled, false if not
	 */
	public static boolean handleActivityResult(Context context,
			PendingCall appCall, int requestCode, Intent data, Callback callback) {
		if (requestCode != appCall.getRequestCode()) {
			return false;
		}

		if (attachmentStore != null) {
			attachmentStore.cleanupAttachmentsForCall(context,
					appCall.getCallId());
		}
	 ...
	 *            the calling Context
	 * @param features
	 *            zero or more features to check for;
	 *            {@link ShareDialogFeature#SHARE_DIALOG} is implicitly checked
	 *            if not explicitly specified
	 * @return true if all of the specified features are supported by the
	 *         currently installed version of the Facebook application; false if
	 *         any of the features are not supported
	 */
	public static boolean canPresentShareDialog(Context context,
			ShareDialogFeature... features) {
		return handleCanPresent(context,
				EnumSet.of(ShareDialogFeature.SHARE_DIALOG, features));
	}

	/**
	 * Determines whether the version of the Facebook application installed on
	 * the user's device is recent enough to support specific features of the
	 * native Message dialog, which in turn may be used to determine which UI,
	 * etc., to present to the user.
	 *
	 * @param context
	 *            the calling Context
	 * @param features
	 *            zero or more features to check for;
	 *            {@link com.facebook.widget.FacebookDialog.MessageDialogFeature#MESSAGE_DIALOG}
	 *            is implicitly checked if not explicitly specified
	 * @return true if all of the specified features are supported by the
	 *         currently installed version of the Facebook application; false if
	 *         any of the features are not supported
	 */
	public static boolean canPresentMessageDialog(Context context,
		...
		return handleCanPresent(context,
				EnumSet.of(MessageDialogFeature.MESSAGE_DIALOG, features));
	}
	 ...
	 * @param features
	 *            zero or more features to check for;
	 *            {@link OpenGraphActionDialogFeature#OG_ACTION_DIALOG} is
	...
	public static boolean canPresentOpenGraphActionDialog(Context context,
		...
		return handleCanPresent(context, EnumSet.of(
				OpenGraphActionDialogFeature.OG_ACTION_DIALOG, features));
	}

	/**
	 * Determines whether the version of the Facebook application installed on
	 * the user's device is recent enough to support specific features of the
	 * native Open Graph Message dialog, which in turn may be used to determine
	 * which UI, etc., to present to the user.
	 *
	 * @param context
	 *            the calling Context
	 * @param features
	 *            zero or more features to check for;
	 *            {@link com.facebook.widget.FacebookDialog.OpenGraphMessageDialogFeature#OG_MESSAGE_DIALOG}
	 *            is implicitly checked if not explicitly specified
	 * @return true if all of the specified features are supported by the
	 *         currently installed version of the Facebook application; false if
	 *         any of the features are not supported
	...
	public static boolean canPresentOpenGraphMessageDialog(Context context,
		...
		return handleCanPresent(context, EnumSet.of(
				OpenGraphMessageDialogFeature.OG_MESSAGE_DIALOG, features));
	}
	...
	private static int getMinVersionForFeatures(
			Iterable<? extends DialogFeature> features) {
		int minVersion = Integer.MIN_VALUE;
		for (DialogFeature feature : features) {
			// Minimum version to support all features is the maximum of each
			// feature's minimum version.
			minVersion = Math.max(minVersion, feature.getMinVersion());
		}
		return minVersion;
	}

	private static String getActionForFeatures(
			Iterable<? extends DialogFeature> features) {
		String action = null;
		for (DialogFeature feature : features) {
			// All actions in a set of DialogFeatures should have the same
			// action
	...
	private static void logDialogActivity(Activity activity, Fragment fragment,
			String eventName, String outcome) {
		AppEventsLogger logger = AppEventsLogger
				.newLogger(fragment != null ? fragment.getActivity() : activity);
	...
	abstract static class Builder<CONCRETE extends Builder<?>> {
		...
		/**
		 * Sets the request code that will be passed to handleActivityResult
		 * when this activity completes; the default is
		...
		/**
		 * Sets the name of the application to be displayed in the dialog. If
		 * provided, this optimizes the user experience as a preview of a shared
		 * item, etc., can be displayed sooner.
		 *
		 * @param applicationName
		 *            the name of the Facebook application
		 * @return this instance of the builder
		 */
		...
		/**
		 * Sets the fragment that should launch the dialog. This allows the
		 * dialog to be launched from a Fragment, and will allow the fragment to
		 * receive the
		...
		/**
		 * Constructs a FacebookDialog with an Intent that is correctly
		 * populated to present the dialog within the Facebook application.
		 *
		...
		public FacebookDialog build() {
			...
			putExtra(extras, NativeProtocol.EXTRA_APPLICATION_NAME,
					applicationName);
			extras = setBundleExtras(extras);

			String action = getActionForFeatures(getDialogFeatures());
			int protocolVersion = getProtocolVersionForNativeDialog(activity,
					action, getMinVersionForFeatures(getDialogFeatures()));

			Intent intent = NativeProtocol.createPlatformActivityIntent(
					activity, action, protocolVersion, extras);
			if (intent == null) {
				logDialogActivity(
						activity,
						fragment,
			...
			return new FacebookDialog(activity, fragment, appCall,
					getOnPresentCallback());
		}

		/**
		 * Determines whether the native dialog can be presented (i.e., whether
		 * the required version of the Facebook application is installed on the
		 * device, and whether the installed version supports all of the
		 * parameters specified for the dialog).
		 *
		...
		OnPresentCallback getOnPresentCallback() {
			return new OnPresentCallback() {
				...
				public void onPresent(Context context) throws Exception {
					// We're actually being presented, so put our attachments in
					// the content provider.
					if (imageAttachments != null && imageAttachments.size() > 0) {
						getAttachmentStore().addAttachmentsForCall(context,
								appCall.getCallId(), imageAttachments);
					}
					if (imageAttachmentFiles != null
							&& imageAttachmentFiles.size() > 0) {
						getAttachmentStore().addAttachmentFilesForCall(context,
								appCall.getCallId(), imageAttachmentFiles);
					}
				}
			};
		}
		...
		protected List<String> addImageAttachments(Collection<Bitmap> bitmaps) {
			...
			for (Bitmap bitmap : bitmaps) {
				...
				String url = NativeAppCallContentProvider.getAttachmentUrl(
						applicationId, appCall.getCallId(), attachmentName);
		...
		protected List<String> addImageAttachmentFiles(
				Collection<File> bitmapFiles) {
			ArrayList<String> attachmentUrls = new ArrayList<String>();
	...
	private abstract static class ShareDialogBuilderBase<CONCRETE extends ShareDialogBuilderBase<?>>
		 ...
		 *
		 * @param activity
		 *            the Activity which is presenting the native Share dialog;
		 *            must not be null
		 */
		 ...
		 *
		 * @param friends
		 *            a list of Facebook IDs of the friends to be tagged in the
		 *            shared item
		 * @return this instance of the builder
		 */
		...
		/**
		 * Sets whether errors encountered during previewing the shared item
		 * should be considered fatal and cause the dialog to return an error
		 *
		 * @param dataErrorsFatal
		...
		Bundle setBundleExtras(Bundle extras) {
			...
			extras.putBoolean(NativeProtocol.EXTRA_DATA_FAILURES_FATAL,
					dataErrorsFatal);
			if (!Utility.isNullOrEmpty(friends)) {
				extras.putStringArrayList(NativeProtocol.EXTRA_FRIEND_TAGS,
						friends);
			}
			return extras;
		}
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Share dialog. This builder will throw an
	 * exception if the Facebook application is not installed, so it should only
	 * be used if
	 * {@link FacebookDialog#canPresentShareDialog(android.content.Context, com.facebook.widget.FacebookDialog.ShareDialogFeature...)}
	...
	private static abstract class PhotoDialogBuilderBase<CONCRETE extends PhotoDialogBuilderBase<?>>
		 ...
		 * <p>
		 * </p>
		 * Adds one or more photos to the list of photos to display in the
		 * native Share dialog, by providing an in-memory representation of the
		 * photos. The dialog's callback will be called once the user has shared
		 * the photos, but the photos themselves may be uploaded in the
		 * background by the Facebook app; apps wishing to be notified when the
		 * photo upload has succeeded or failed should extend the
		 * FacebookBroadcastReceiver class and register it in their
		 * AndroidManifest.xml.</p>
		 * <p>
		 * In order for the images to be provided to the Facebook application as
		 * part of the app call, the NativeAppCallContentProvider must be
		 * specified correctly in the application's AndroidManifest.xml.
		 * </p>
		 * No more than six photos may be shared at a time.
		 *
		 * @param photos
		 *            a collection of Files representing photos to be uploaded
		 * @return this instance of the builder
		 */
		...
		/**
		 * Adds one or more photos to the list of photos to display in the
		 * native Share dialog, by specifying their location in the file system.
		 * The dialog's callback will be called once the user has shared the
		 * photos, but the photos themselves may be uploaded in the background
		 * by the Facebook app; apps wishing to be notified when the photo
		 * upload has succeeded or failed should extend the
		 * FacebookBroadcastReceiver class and register it in their
		 * AndroidManifest.xml. No more than six photos may be shared at a time.
		 *
		 * @param photos
		 *            a collection of Files representing photos to be uploaded
		 * @return this instance of the builder
		...
		Bundle setBundleExtras(Bundle extras) {
			...
			putExtra(extras, NativeProtocol.EXTRA_APPLICATION_NAME,
					applicationName);
			putExtra(extras, NativeProtocol.EXTRA_PLACE_TAG, place);
			extras.putStringArrayList(NativeProtocol.EXTRA_PHOTOS,
					imageAttachmentUrls);
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Share dialog for sharing photos. This builder
	 * will throw an exception if the Facebook application is not installed, so
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Message dialog for sharing photos. This builder
	 * will throw an exception if the Messenger application is not installed, so
	 * it should only be used if
	 * {@link FacebookDialog#canPresentMessageDialog(android.content.Context, com.facebook.widget.FacebookDialog.MessageDialogFeature...)}
	 * indicates the capability is available.
	 */
	public static class PhotoMessageDialogBuilder extends
		 ...
		 *
		 * @param activity
		 *            the Activity which is presenting the native Message
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Message dialog. This builder will throw an
	 * exception if the Facebook Messenger application is not installed, so it
	 * should only be used if
	 * {@link FacebookDialog#canPresentMessageDialog(android.content.Context, com.facebook.widget.FacebookDialog.MessageDialogFeature...)}
	 * indicates the capability is available. The "friends" and "place"
	 * properties will be ignored as the Facebook Messenger app does not support
	 * tagging.
	 */
	...
	private static abstract class OpenGraphDialogBuilderBase<CONCRETE extends OpenGraphDialogBuilderBase<?>>
		 ...
		 *
		 * @param activity
		 *            the Activity which is presenting the native Open Graph
		 *            action publish dialog; must not be null
		 * @param action
		 *            the Open Graph action to be published, which must contain
		 *            a reference to at least one Open Graph object with the
		 *            property name specified by setPreviewPropertyName; the
		 *            action must have had its type specified via the
		 *            {@link OpenGraphAction#setType(String)} method
		 * @param actionType
		 *            the type of the Open Graph action to be published, which
		 *            should be the namespace-qualified name of the action type
		 *            (e.g., "myappnamespace:myactiontype"); this will override
		 *            the type of the action passed in.
		 * @param previewPropertyName
		 *            the name of a property on the Open Graph action that
		 *            contains the Open Graph object which will be displayed as
		 *            a preview to the user
		 */
		@Deprecated
		public OpenGraphDialogBuilderBase(Activity activity,
				OpenGraphAction action, String actionType,
			...
			if (!Utility.isNullOrEmpty(typeOnAction)
					&& !typeOnAction.equals(actionType)) {
				throw new IllegalArgumentException(
		 ...
		 */
		public OpenGraphDialogBuilderBase(Activity activity,
				OpenGraphAction action, String previewPropertyName) {
			super(activity);
		...
		/**
		 * <p>
		 * Specifies a list of images for the Open Graph action that should be
		 * uploaded prior to publishing the action. The action must already have
		 * been set prior to calling this method. This method will generate
		 * unique names for the image attachments and update the action to refer
		 * to these attachments. Note that calling setAction again after calling
		 ...
		 * images will not be marked as being user-generated.
		 * </p>
		 * <p/>
		 * <p>
		 * In order for the images to be provided to the Facebook application as
		 ...
		 *
		 * @param bitmaps
		 *            a list of Bitmaps to be uploaded and attached to the Open
		 *            Graph action
		 * @return this instance of the builder
		 */
		 ...
		 * this method will not clear the image attachments already set, but the
		 * new action will have no reference to the existing attachments. The
		 * images may be marked as being user-generated -- refer to <a href=
		 ...
		 *            if true, specifies that the user_generated flag should be
		...
		public CONCRETE setImageAttachmentsForAction(List<Bitmap> bitmaps,
			...
			if (action == null) {
				throw new FacebookException(
						"Can not set attachments prior to setting action.");
			}
		 ...
		 * Specifies a list of images for the Open Graph action that should be
		 * uploaded prior to publishing the action. The action must already have
		 * been set prior to calling this method. The images will not be marked
		 * as being user-generated. This method will generate unique names for
		 * the image attachments and update the action to refer to these
		 * attachments. Note that calling setAction again after calling this
		 * method will not clear the image attachments already set, but the new
		 ...
		 *
		 * @param bitmapFiles
		 *            a list of Files containing bitmaps to be uploaded and
		 *            attached to the Open Graph action
		 * @return this instance of the builder
		 */
		 ...
		 * Specifies a list of images for the Open Graph action that should be
		 * uploaded prior to publishing the action. The action must already have
		 * been set prior to calling this method. The images may be marked as
		 * being user-generated -- refer to <a href=
		 * "https://developers.facebook.com/docs/opengraph/howtos/adding-photos-to-stories/"
		 * >this article</a> for more information. This method will generate
		 * unique names for the image attachments and update the action to refer
		 * to these attachments. Note that calling setAction again after calling
		 * this method will not clear the image attachments already set, but the
		 * new action will have no reference to the existing attachments.
		 * </p>
		 * <p/>
		 * <p>
		 * In order for the images to be provided to the Facebook application as
		 ...
		 * @return this instance of the builder
		 */
		public CONCRETE setImageAttachmentFilesForAction(
		...
		private void updateActionAttachmentUrls(List<String> attachmentUrls,
			...
			for (String url : attachmentUrls) {
				...
				try {
					...
					if (isUserGenerated) {
						jsonObject.put(NativeProtocol.IMAGE_USER_GENERATED_KEY,
								true);
					}
		...
		/**
		 * <p>
		 * Specifies a list of images for an Open Graph object referenced by the
		 * action that should be uploaded prior to publishing the action. The
		 * images will not be marked as user-generated. The action must already
		 * have been set prior to calling this method, and the action must have
		 * a GraphObject-valued property with the specified property name. This
		 * method will generate unique names for the image attachments and
		 * update the graph object to refer to these attachments. Note that
		 * calling setObject again after calling this method, or modifying the
		 ...
		 *            the name of a property on the action that corresponds to
		 *            an Open Graph object; the object must be marked as a new
		 *            object to be created (i.e.,
		 *            {@link com.facebook.model.OpenGraphObject#getCreateObject()}
		 *            must return true) or an exception will be thrown
		 * @param bitmaps
		 *            a list of Files containing bitmaps to be uploaded and
		 *            attached to the Open Graph object
		 * @return this instance of the builder
		 */
		public CONCRETE setImageAttachmentsForObject(String objectProperty,
				List<Bitmap> bitmaps) {
		 ...
		 * images may be marked as being user-generated -- refer to <a href=
		 * "https://developers.facebook.com/docs/opengraph/howtos/adding-photos-to-stories/"
		 * >this article</a> for more information. The action must already have
		 * been set prior to calling this method, and the action must have a
		 * GraphObject-valued property with the specified property name. This
		 * method will generate unique names for the image attachments and
		 * update the graph object to refer to these attachments. Note that
		 * calling setObject again after calling this method, or modifying the
		 * value of the specified property, will not clear the image attachments
		 ...
		 */
		public CONCRETE setImageAttachmentsForObject(String objectProperty,
				List<Bitmap> bitmaps, boolean isUserGenerated) {
			...
			List<String> attachmentUrls = addImageAttachments(bitmaps);
			updateObjectAttachmentUrls(objectProperty, attachmentUrls,
					isUserGenerated);
		...
		public CONCRETE setImageAttachmentFilesForObject(String objectProperty,
			...
			return setImageAttachmentFilesForObject(objectProperty,
					bitmapFiles, false);
		}
		...
		void updateObjectAttachmentUrls(String objectProperty,
				List<String> attachmentUrls, boolean isUserGenerated) {
			final OpenGraphObject object;
			try {
				object = action.getPropertyAs(objectProperty,
						OpenGraphObject.class);
				if (object == null) {
					throw new IllegalArgumentException(
							"Action does not contain a property '"
									+ objectProperty + "'");
				}
			} catch (FacebookGraphObjectException exception) {
				throw new IllegalArgumentException("Property '"
						+ objectProperty + "' is not a graph object");
			}
			if (!object.getCreateObject()) {
				throw new IllegalArgumentException("The Open Graph object in '"
						+ objectProperty + "' is not marked for creation");
			}
			...
			for (String url : attachmentUrls) {
				...
				if (isUserGenerated) {
					graphObject.setProperty(
							NativeProtocol.IMAGE_USER_GENERATED_KEY, true);
				}
		...
		@Override
		Bundle setBundleExtras(Bundle extras) {
			putExtra(extras, NativeProtocol.EXTRA_PREVIEW_PROPERTY_NAME,
					previewPropertyName);
		...
		private JSONObject flattenChildrenOfGraphObject(JSONObject graphObject) {
			try {
				// Clone the existing object to avoid modifying it from under
				// the caller.
		...
		private Object flattenObject(Object object) throws JSONException {
			...
			if (object instanceof JSONObject) {
				...
				if (jsonObject
						.optBoolean(NativeProtocol.OPEN_GRAPH_CREATE_OBJECT_KEY)) {
					return object;
				}
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Open Graph action publish dialog. This builder
	 * allows the caller to specify binary images for both the action and any
	 * Open Graph objects to be created prior to publishing the action. This
	 * builder will throw an exception if the Facebook application is not
	 * installed, so it should only be used if
	 * {@link FacebookDialog#canPresentOpenGraphActionDialog(android.content.Context, com.facebook.widget.FacebookDialog.OpenGraphActionDialogFeature...)}
	 * indicates the capability is available.
	 */
	...
	/**
	 * Provides a builder which can construct a FacebookDialog instance suitable
	 * for presenting the native Open Graph action message dialog. This builder
	 * allows the caller to specify binary images for both the action and any
	 * Open Graph objects to be created prior to publishing the action. This
	 * builder will throw an exception if the Facebook application is not
	 * installed, so it should only be used if
	...
	public static class OpenGraphMessageDialogBuilder extends
		 ...
		 * @param activity
		 *            the Activity which is presenting the native Open Graph
		 *            action message dialog; must not be null
		 * @param action
		 *            the Open Graph action to be sent, which must contain a
		 *            reference to at least one Open Graph object with the
		 *            property name specified by setPreviewPropertyName; the
		 *            action must have had its type specified via the
		 *            {@link OpenGraphAction#setType(String)} method
		 * @param previewPropertyName
	...
	/**
	 * Encapsulates information about a call being made to the Facebook
	 * application for Android. A unique String call ID is used to track calls
	...
	public static class PendingCall implements Parcelable {
		 ...
		 *
		 * @param requestCode
		 *            the request code for this app call
		 */
		...
		private void setRequestIntent(Intent requestIntent) {
			...
			this.requestIntent.putExtra(NativeProtocol.EXTRA_PROTOCOL_CALL_ID,
					callId.toString());
		}
		 ...
		 *
		 * @return the request code that will be passed to handleActivityResult
		 *         upon completion.
		 */
