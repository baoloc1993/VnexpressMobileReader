 ...
 * <p>
 * This class should also be used by Activities that will be displaying native
 * dialogs provided by the Facebook application, in order to handle processing
 * of the activity results generated by those dialogs.
 */
public class UiLifecycleHelper {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			pendingFacebookDialogCall = savedInstanceState
	 ...
	 *            the callback for handling FacebookDialog results, can be null
	 */
	public void onActivityResult(int requestCode, int resultCode, Intent data,
		...
		handleFacebookDialogActivityResult(requestCode, resultCode, data,
	...
	public void onSaveInstanceState(Bundle outState) {
		...
		outState.putParcelable(DIALOG_CALL_BUNDLE_SAVE_KEY,
	...
	/**
	 * Register that we are expecting results from a call to the Facebook
	 * application (e.g., from a native dialog provided by the Facebook app).
	 * Activity results forwarded to onActivityResults will be parsed and
	 * handled if they correspond to this call. Only a single pending
	 * FacebookDialog call can be tracked at a time; attempting to track another
	 * one will cancel the first one.
	 *
	 * @param appCall
	...
	public void trackPendingDialogCall(FacebookDialog.PendingCall pendingCall) {
		if (pendingFacebookDialogCall != null) {
			// If one is already pending, cancel it; we don't allow multiple
			// pending calls.
			Log.i("Facebook",
					"Tracking new app call while one is still pending; canceling pending call.");
	...
	/**
	 * Retrieves an instance of AppEventsLogger that can be used for the current
	 * Session, if any. Different instances may be returned if the current
	 * Session changes, so this value should not be cached for long periods of
	...
	public AppEventsLogger getAppEventsLogger() {
		...
		if (appEventsLogger == null
				|| !appEventsLogger.isValidForSession(session)) {
			if (appEventsLogger != null) {
				// Pretend we got stopped so the old logger will persist its
				// results now, in case we get stopped
	...
	private boolean handleFacebookDialogActivityResult(int requestCode,
			int resultCode, Intent data,
			FacebookDialog.Callback facebookDialogCallback) {
		if (pendingFacebookDialogCall == null
				|| pendingFacebookDialogCall.getRequestCode() != requestCode) {
			return false;
		}

		if (data == null) {
			// We understand the request code, but have no Intent. This can
			// happen if the called Activity crashes
			// before it can be started; we treat this as a cancellation because
			// we have no other information.
			cancelPendingAppCall(facebookDialogCallback);
			return true;
		}

		String callIdString = data
		...
		if (callId != null
				&& pendingFacebookDialogCall.getCallId().equals(callId)) {
			// Yes, we can handle it normally.
			FacebookDialog.handleActivityResult(activity,
					pendingFacebookDialogCall, requestCode, data,
					facebookDialogCallback);
		} else {
			// No, send a cancellation error to the pending call and ignore the
			// result, because we
	...
	private void cancelPendingAppCall(
			FacebookDialog.Callback facebookDialogCallback) {
