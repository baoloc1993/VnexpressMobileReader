 ...
 * should instead use {@link Session} to manage session state, {@link Request}
 * to make API requests, and {@link com.facebook.widget.WebDialog} to make
 * dialog requests.
 * <p/>
...
public class Facebook {
	 ...
	 * <p/>
	 * This method is deprecated. See {@link Facebook} and {@link Session} for
	 * more info.
	 */
	@Deprecated
	...
	private void authorize(Activity activity, String[] permissions,
			int activityCode, SessionLoginBehavior behavior,
			final DialogListener listener) {
		...
		StatusCallback callback = new StatusCallback() {
			@Override
			public void call(Session callbackSession, SessionState state,
					Exception exception) {
	...
	private void openSession(Session session, Session.OpenRequest openRequest,
			boolean isPublish) {
	...
	private void onSessionCallback(Session callbackSession, SessionState state,
			Exception exception, DialogListener listener) {
	...
	/**
	 * IMPORTANT: If you are using the deprecated authorize() method, this
	 * method must be invoked at the top of the calling activity's
	 * onActivityResult() function or Facebook authentication will not function
	 * properly!
	 * <p/>
	...
	public void authorizeCallback(int requestCode, int resultCode, Intent data) {
		...
		if (pending != null) {
			if (pending.onActivityResult(this.pendingAuthorizationActivity,
	...
	public boolean shouldExtendAccessToken() {
		...
		return isSessionValid()
				&& (System.currentTimeMillis()
						- lastAccessUpdateMillisecondsAfterEpoch >= REFRESH_TOKEN_BARRIER);
	}
	...
	private static class TokenRefreshConnectionHandler extends Handler {
		...
		TokenRefreshConnectionHandler(Facebook facebook,
				TokenRefreshServiceConnection connection) {
			super();
		...
		public void handleMessage(Message msg) {
			...
			TokenRefreshServiceConnection connection = connectionWeakReference
					.get();
			...
			// Legacy functions in Facebook class (and ServiceListener
			// implementors) expect expires_in in
			...
			if (token != null) {
				...
				if (refreshSession != null) {
					// Session.internalRefreshToken expects the original bundle
					// with expires_in in seconds from
					// epoch.
					LegacyHelper.extendTokenCompleted(refreshSession,
							msg.getData());
				}

				if (connection.serviceListener != null) {
					// To avoid confusion we should return the expiration time
					// in
			...
			} else if (connection.serviceListener != null) { // extract errors
				// only if
				// client wants them
	 ...
	 * </code>
	 * <p/>
	 * This method is deprecated. See {@link Facebook} and {@link Request} for
	 * more info.
	 *
	 * @param parameters
	...
	public void dialog(Context context, String action, Bundle parameters,
		...
		if (action.equals(LOGIN)) {
			...
			// We do not want to add an access token when displaying the auth
			// dialog.
			if (isSessionValid()) {
	...
	/**
	 * Allows the user to set a Session for the Facebook class to use. If a
	 * Session is set here, then one should not use the authorize, logout, or
	 * extendAccessToken methods which alter the Session object since that may
	 * result in undefined behavior. Using those methods after setting the
	 * session here will result in exceptions being thrown.
	 *
	 * @param session
	 *            the Session object to use, cannot be null
	 */
	@Deprecated
	...
	private void checkUserSession(String methodName) {
		if (userSetSession != null) {
			throw new UnsupportedOperationException(String.format(
					"Cannot call %s after setSession has been called.",
					methodName));
		}
	}
	...
	public void setAccessExpiresIn(String expiresInSecsFromNow) {
		...
		if (expiresInSecsFromNow != null) {
			long expires = expiresInSecsFromNow.equals("0") ? 0 : System
					.currentTimeMillis()
					+ Long.parseLong(expiresInSecsFromNow)
					* 1000L;
	 ...
	 *
	 * @param appId
	 *            the String representing the application ID
	 */
	@Deprecated
	...
	private class SetterTokenCachingStrategy extends TokenCachingStrategy {
		...
		public Bundle load() {
			...
			if (accessToken != null) {
				...
				TokenCachingStrategy.putSource(bundle,
		...
		public void save(Bundle bundle) {
			...
			accessExpiresMillisecondsAfterEpoch = TokenCachingStrategy
					.getExpirationMilliseconds(bundle);
			pendingAuthorizationPermissions = stringArray(TokenCachingStrategy
	 ...
	 * <p/>
	 * This method is deprecated. See {@link Facebook} and {@link Settings} for
	 * more info.
	 *
	 ...
	 * more info.
	 *
	 * @return a Boolean indicating whether installation of the app should be
	 *         auto-published.
	 */
	@Deprecated
	 ...
	 *
	 * @param value
	 *            a Boolean indicating whether installation of the app should be
	 *            auto-published.
	 */
	@Deprecated
	 ...
	 * @return Always false. Earlier versions of the API returned true if it was
	 *         no longer necessary to call. Apps should ignore this value, but
	 *         for compatibility we will return false to ensure repeat calls
	 *         (and the underlying code will prevent duplicate network traffic).
	 */
	@Deprecated
	 ...
	 * <p/>
	 * All public members of this class are intentionally deprecated. New code
	 * should instead use {@link com.facebook.widget.WebDialog}
	 * <p/>
	 * Adding @Deprecated to this class causes warnings in other deprecated
	 * classes that reference this one. That is the only reason this entire
	 ...
	 * <p/>
	 * All public members of this class are intentionally deprecated. New code
	 * should instead use {@link Session} to manage session state.
	 * <p/>
	 * Adding @Deprecated to this class causes warnings in other deprecated
	 * classes that reference this one. That is the only reason this entire
	 * class is not deprecated.
