...
public class LoginActivity extends Activity {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (savedInstanceState != null) {
			callingPackage = savedInstanceState
					.getString(SAVED_CALLING_PKG_KEY);
			authorizationClient = (AuthorizationClient) savedInstanceState
					.getSerializable(SAVED_AUTH_CLIENT);
		} else {
			callingPackage = getCallingPackage();
			authorizationClient = new AuthorizationClient();
			request = (AuthorizationClient.AuthorizationRequest) getIntent()
	...
	private void onAuthClientCompleted(AuthorizationClient.Result outcome) {
		...
		int resultCode = (outcome.code == AuthorizationClient.Result.Code.CANCEL) ? RESULT_CANCELED
				: RESULT_OK;
	...
	public void onResume() {
		...
		// If the calling package is null, this generally means that the callee
		// was started
		// with a launchMode of singleInstance. Unfortunately, Android does not
		// allow a result
		// to be set when the callee is a singleInstance, so we log an error and
		// return.
	...
	static Bundle populateIntentExtras(
