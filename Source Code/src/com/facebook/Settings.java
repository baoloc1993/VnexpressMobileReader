...
public final class Settings {
	...
	/**
	 * loadDefaultsFromMetadata will attempt to load certain settings (e.g.,
	 * application ID, client token) from metadata in the app's
	 * AndroidManifest.xml. The application ID will be read from this key.
	 */
	public static final String APPLICATION_ID_PROPERTY = "com.facebook.sdk.ApplicationId";
	/**
	 * loadDefaultsFromMetadata will attempt to load certain settings (e.g.,
	 * application ID, client token) from metadata in the app's
	 * AndroidManifest.xml. The client token will be read from this key.
	 */
	...
	public static final Set<LoggingBehavior> getLoggingBehaviors() {
		synchronized (loggingBehaviors) {
			return Collections.unmodifiableSet(new HashSet<LoggingBehavior>(
					loggingBehaviors));
		}
	}
	 ...
	 */
	public static final boolean isLoggingBehaviorEnabled(
	...
	/**
	 * Gets the base Facebook domain to use when making Web requests; in
	 * production code this will always be "facebook.com".
	 *
	...
	/**
	 * Sets the base Facebook domain to use when making Web requests. This
	 * defaults to "facebook.com", but may be overridden to, e.g.,
	 * "beta.facebook.com" to direct requests at a different domain. This method
	 * should never be called from production code.
	 *
	 * @param facebookDomain
	 *            the base domain to use instead of "facebook.com"
	 */
	public static void setFacebookDomain(String facebookDomain) {
		if (!BuildConfig.DEBUG) {
			Log.w(TAG,
					"WARNING: Calling setFacebookDomain from non-DEBUG code.");
		}
	 ...
	 *            {@link AppEventsLogger#activateApp(Context, String)} for more
	 *            info.
	 */
	@Deprecated
	public static void publishInstallAsync(final Context context,
	...
	@Deprecated
	public static void publishInstallAsync(final Context context,
			final String applicationId, final Request.Callback callback) {
		// grab the application context ahead of time, since we will return to
		// the caller immediately.
		final Context applicationContext = context.getApplicationContext();
		Settings.getExecutor().execute(new Runnable() {
			@Override
			public void run() {
				final Response response = Settings
						.publishInstallAndWaitForResponse(applicationContext,
								applicationId);
				if (callback != null) {
	 ...
	 *
	 * @param shouldAutoPublishInstall
	 ...
	 */
	@Deprecated
	public static void setShouldAutoPublishInstall(
			boolean shouldAutoPublishInstall) {
	 ...
	 */
	@Deprecated
	public static boolean publishInstallAndWait(final Context context,
			final String applicationId) {
		Response response = publishInstallAndWaitForResponse(context,
				applicationId);
	 ...
	 */
	@Deprecated
	public static Response publishInstallAndWaitForResponse(
			final Context context, final String applicationId) {
		return publishInstallAndWaitForResponse(context, applicationId, false);
	}

	static Response publishInstallAndWaitForResponse(final Context context,
			final String applicationId, final boolean isAutoPublish) {
		try {
			if (context == null || applicationId == null) {
				throw new IllegalArgumentException(
						"Both context and applicationId must be non-null");
			}
			AttributionIdentifiers identifiers = AttributionIdentifiers
			...
			publishParams.setProperty("application_package_name",
					context.getPackageName());

			String publishUrl = String.format(PUBLISH_ACTIVITY_PATH,
					applicationId);
			Request publishRequest = Request.newPostRequest(null, publishUrl,
					publishParams, null);
			...
			if (lastPing != 0) {
				...
				try {
					if (lastResponseJSON != null) {
						graphObject = GraphObject.Factory
								.create(new JSONObject(lastResponseJSON));
					}
				} catch (JSONException je) {
					// return the default graph object if there is any problem
					// reading the data.
				}
				if (graphObject == null) {
					return Response.createResponsesFromString("true", null,
							new RequestBatch(publishRequest), true).get(0);
				} else {
					return new Response(null, null, null, graphObject, true);
				}
			} else if (identifiers.getAndroidAdvertiserId() == null
					&& identifiers.getAttributionId() == null) {
				throw new FacebookException(
						"No attribution id available to send to server.");
			} else {
				if (!Utility.queryAppSettings(applicationId, false)
						.supportsAttribution()) {
					throw new FacebookException(
							"Install attribution has been disabled on the server.");
				}
				...
				// if we got an object response back, cache the string of the
				// JSON.
	...
	public static String getAttributionId(ContentResolver contentResolver) {
		try {
			...
			Log.d(TAG, "Caught unexpected exception in getAttributionId(): "
					+ e.toString());
			return null;
		}
	}
	 ...
	 * AppEventsLogger.logEvent calls logs its event with the current app
	 * version, and App Insights allows breakdown of events by app version.
	 *
	 * @param appVersion
	 *            The version identifier of the Android app that events are
	 *            being logged through. Enables analysis and breakdown of logged
	...
	/**
	 * Gets whether data such as that generated through AppEventsLogger and sent
	 * to Facebook should be restricted from being used for purposes other than
	 * analytics and conversions, such as for targeting ads to this user.
	 * Defaults to false. This value is stored on the device and persists across
	 * app launches.
	 *
	 * @param context
	...
	/**
	 * Sets whether data such as that generated through AppEventsLogger and sent
	 * to Facebook should be restricted from being used for purposes other than
	 * analytics and conversions, such as for targeting ads to this user.
	 * Defaults to false. This value is stored on the device and persists across
	 * app launches. Changes to this setting will apply to app events currently
	 * queued to be flushed.
	 *
	 * @param context
	 *            Used to persist this value across app runs.
	 */
	...
	/**
	 * Sets the threshold used to report progress on requests. Note that the
	 * value will be read when the request is started and can not be changed
	 * during a request (or batch) execution.
	 *
	 * @param threshold
	 *            The number of bytes progressed to force a callback.
	 */
	 ...
	 * making calls to v1.0 endpoints by default) The default is false.
	 *
	 * @return the value
	 */
	 ...
	 * making calls to v1.0 endpoints by default) The default is false. This is
	 * provided for apps that have strong reason not to take advantage of new
	 * capabilities in version 2.0+ of the API.
	 *
	 * @param platformCompatibilityEnabled
	 *            whether to set Legacy Graph API mode
	 */
	public static void setPlatformCompatibilityEnabled(
			boolean platformCompatibilityEnabled) {
		Settings.platformCompatibilityEnabled = platformCompatibilityEnabled;
	}

	/**
	 * Loads default values for certain settings from an application's
	 * AndroidManifest.xml metadata, if possible. If values have been explicitly
	 * set for a particular setting, they will not be overwritten. The following
	 * settings are currently loaded from metadata: APPLICATION_ID_PROPERTY,
	 * CLIENT_TOKEN_PROPERTY
	 *
	 * @param context
	 *            the Context to use for loading metadata
	 */
	...
	/**
	 * Gets the Facebook application ID for the current app. This will be null
	 * unless explicitly set or unless loadDefaultsFromMetadata has been called.
	 *
	...
	/**
	 * Gets the client token for the current app. This will be null unless
	 * explicitly set or unless loadDefaultsFromMetadata has been called.
	 *
