...
public final class NativeProtocol {
	...
	// OG objects will have this key to set to true if they should be created as
	// part of OG Action publish
	...
	// Keys for status data in MESSAGE_ERROR_REPLY from PlatformService and for
	// error
	...
	private static abstract class NativeAppInfo {
		...
		public boolean validateSignature(Context context, String packageName) {
			...
			if (brand.startsWith("generic")
				...
				// We are debugging on an emulator, don't validate package
				// signature.
				return true;
			}
			...
			for (Signature signature : packageInfo.signatures) {
				String hashedSignature = Utility.sha1hash(signature
						.toByteArray());
	...
	private static List<NativeAppInfo> buildFacebookAppList() {
		...
		// Katana needs to be the first thing in the list since it will get
		// selected as the default FACEBOOK_APP_INFO
	...
	static Intent validateActivityIntent(Context context, Intent intent,
		...
		if (!appInfo.validateSignature(context,
	...
	static Intent validateServiceIntent(Context context, Intent intent,
			NativeAppInfo appInfo) {
		if (intent == null) {
			return null;
		}
	...
	public static Intent createProxyAuthIntent(Context context,
			String applicationId, List<String> permissions, String e2e,
			boolean isRerequest) {
		for (NativeAppInfo appInfo : facebookAppInfoList) {
			...
			intent.putExtra(ServerProtocol.DIALOG_PARAM_RESPONSE_TYPE,
					ServerProtocol.DIALOG_RESPONSE_TYPE_TOKEN);
			intent.putExtra(ServerProtocol.DIALOG_PARAM_RETURN_SCOPES,
					ServerProtocol.DIALOG_RETURN_SCOPES_TRUE);

			if (!Settings.getPlatformCompatibilityEnabled()) {
				// Override the API Version for Auth
				intent.putExtra(ServerProtocol.DIALOG_PARAM_LEGACY_OVERRIDE,
						ServerProtocol.GRAPH_API_VERSION);

				// Only set the rerequest auth type for non legacy requests
				if (isRerequest) {
					intent.putExtra(ServerProtocol.DIALOG_PARAM_AUTH_TYPE,
	...
	// Note: be sure this stays sorted in descending order; add new versions at
	// the beginning
	private static final List<Integer> KNOWN_PROTOCOL_VERSIONS = Arrays.asList(
			PROTOCOL_VERSION_20140324, PROTOCOL_VERSION_20140204,
			PROTOCOL_VERSION_20131107, PROTOCOL_VERSION_20130618,
			PROTOCOL_VERSION_20130502, PROTOCOL_VERSION_20121101);

	private static Intent findActivityIntent(Context context,
			String activityAction, String internalAction) {
		...
		for (NativeAppInfo appInfo : list) {
			intent = new Intent().setAction(activityAction)
	...
	public static Intent createPlatformActivityIntent(Context context,
			String action, int version, Bundle extras) {
		Intent intent = findActivityIntent(context,
				INTENT_ACTION_PLATFORM_ACTIVITY, action);
		if (intent == null) {
			return null;
		}
	...
	public static Exception getErrorFromResult(Intent resultIntent) {
		...
		String description = resultIntent
				.getStringExtra(STATUS_ERROR_DESCRIPTION);
	...
	public static int getLatestAvailableProtocolVersionForService(
			Context context, final int minimumVersion) {
		// Services are currently always against the Facebook App
		return getLatestAvailableProtocolVersionForAppInfoList(context,
				facebookAppInfoList, minimumVersion);
	}

	public static int getLatestAvailableProtocolVersionForAction(
			Context context, String action, final int minimumVersion) {
		List<NativeAppInfo> appInfoList = actionToAppInfoMap.get(action);
		return getLatestAvailableProtocolVersionForAppInfoList(context,
				appInfoList, minimumVersion);
	}

	private static int getLatestAvailableProtocolVersionForAppInfoList(
			Context context, List<NativeAppInfo> appInfoList,
		...
		for (NativeAppInfo appInfo : appInfoList) {
			int protocolVersion = getLatestAvailableProtocolVersionForAppInfo(
					context, appInfo, minimumVersion);
	...
	private static int getLatestAvailableProtocolVersionForAppInfo(
			Context context, NativeAppInfo appInfo, final int minimumVersion) {
		...
		try {
			...
			while (c.moveToNext()) {
				int version = c.getInt(c
						.getColumnIndex(PLATFORM_PROVIDER_VERSION_COLUMN));
	...
	private static Uri buildPlatformProviderVersionURI(NativeAppInfo appInfo) {
		return Uri.parse(CONTENT_SCHEME + appInfo.getPackage()
