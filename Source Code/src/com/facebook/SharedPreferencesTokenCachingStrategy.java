...
public class SharedPreferencesTokenCachingStrategy extends TokenCachingStrategy {
	...
	private static final String TAG = SharedPreferencesTokenCachingStrategy.class
			.getSimpleName();
	 ...
	 * SharedPreferencesTokenCachingStrategy} instance that provides access to a
	 * single set of token information.
	 *
	 * @param context
	 ...
	 * SharedPreferencesTokenCachingStrategy} instance that is distinct for the
	 * passed in cacheKey.
	 *
	 * @param context
	 *            The Context object to use to get the SharedPreferences object.
	...
	public SharedPreferencesTokenCachingStrategy(Context context,
		...
		this.cacheKey = Utility.isNullOrEmpty(cacheKey) ? DEFAULT_CACHE_KEY
				: cacheKey;

		// If the application context is available, use that. However, if it
		// isn't
		// available (possibly because of a context that was created manually),
		// use
		// the passed in context directly.
		Context applicationContext = context.getApplicationContext();
		context = applicationContext != null ? applicationContext : context;

		this.cache = context.getSharedPreferences(this.cacheKey,
		...
		for (String key : allCachedEntries.keySet()) {
			try {
				...
				Logger.log(LoggingBehavior.CACHE, Log.WARN, TAG,
						"Error reading cached value for key: '" + key + "' -- "
								+ e);
				return null;
			}
		}
	...
	public void save(Bundle bundle) {
		...
		for (String key : bundle.keySet()) {
			try {
				...
				// Bypass the commit and just return. This cancels the entire
				// edit transaction
				return;
			}
		}
	...
	private void serializeKey(String key, Bundle bundle,
		...
		if (value instanceof Byte) {
			...
			// Optimistically create a JSONArray. If not an array type, we can
			// null
			// it out later
			...
			if (value instanceof byte[]) {
				...
				// Unsupported type. Clear out the array as a precaution even
				// though
		...
		if (supportedType != null) {
			...
			if (jsonArray != null) {
				// If we have an array, it has already been converted to JSON.
				// So use
				// that instead.
	...
	private void deserializeKey(String key, Bundle bundle) throws JSONException {
		...
		if (valueType.equals(TYPE_BOOLEAN)) {
			...
			for (int i = 0; i < numStrings; i++) {
				...
				stringList.add(i, jsonStringValue == JSONObject.NULL ? null
			...
			try {
				...
				Class<? extends Enum> enumClass = (Class<? extends Enum>) Class
				.forName(enumType);
				@SuppressWarnings("unchecked")
				Enum<?> enumValue = Enum.valueOf(enumClass,
						json.getString(JSON_VALUE));
