 ...
 * Session is created, it attempts to initialize itself from a
 * TokenCachingStrategy. Closing the session can optionally clear this cache.
 * The Session lifecycle uses {@link SessionState SessionState} to indicate its
 * state. Once a Session has been closed, it can't be re-opened; a new Session
 * must be created.
 * </p>
 * <p>
...
public class Session implements Serializable {
	...
	Session(Context context, String applicationId,
	...
	Session(Context context, String applicationId,
		...
		if (tokenCachingStrategy == null) {
			tokenCachingStrategy = new SharedPreferencesTokenCachingStrategy(
					staticContext);
		}
		...
		Bundle tokenState = loadTokenFromCache ? tokenCachingStrategy.load()
				: null;
		if (TokenCachingStrategy.hasTokenInformation(tokenState)) {
			Date cachedExpirationDate = TokenCachingStrategy.getDate(
					tokenState, TokenCachingStrategy.EXPIRATION_DATE_KEY);
			Date now = new Date();
	...
	public final Date getExpirationDate() {
		synchronized (this.lock) {
			return (this.tokenInfo == null) ? null : this.tokenInfo
					.getExpires();
		}
	}
	 ...
	 * {@link #requestNewReadPermissions} or
	 * {@link #requestNewPublishPermissions}.
	 * </p>
	 *
	...
	/**
	 * <p>
	 * Returns the list of permissions that have been requested in this session
	 * but not granted
	 * </p>
	 *
	 ...
	 *             if any publish or manage permissions are requested
	 */
	 ...
	 * <p>
	 * The permissions associated with the openRequest passed to this method
	 * must be publish or manage permissions only and must be non-empty. Any
	 * read permissions will result in a warning, and may fail during
	 * server-side authorization. Also, an application must have at least basic
	 * read permissions prior to requesting publish permissions, so this method
	 * should only be used if the application knows that the user has already
	 * granted read permissions to the application; otherwise, openForRead
	 * should be used, followed by a call to requestNewPublishPermissions. For
	 * more information on this flow, see
	 ...
	 * Session/TokenCachingStrategy-based session-management system. An example
	 ...
	 * in again when upgrading to a version of the app that uses the SDK.
	 * <p/>
	 ...
	 * </p>
	 * <p>
	 * The permissions associated with the newPermissionsRequest passed to this
	 ...
	 * </p>
	 *
	 * @param newPermissionsRequest
	 *            the new permissions request
	 */
	public final void requestNewReadPermissions(
			NewPermissionsRequest newPermissionsRequest) {
		requestNewPermissions(newPermissionsRequest,
	 ...
	 * <p>
	 * The permissions associated with the newPermissionsRequest passed to this
	 * method must be publish or manage permissions only and must be non-empty.
	 * Any read permissions will result in a warning, and may fail during
	 ...
	 *            the new permissions request
	 */
	public final void requestNewPublishPermissions(
			NewPermissionsRequest newPermissionsRequest) {
		requestNewPermissions(newPermissionsRequest,
	 ...
	 * </p>
	 * <p>
	 * If successful, this will update the permissions and call the app back
	 * with {@link SessionState#OPENED_TOKEN_UPDATED}. The session can then be
	 * queried to see the granted and declined permissions. If this fails
	 * because the user has removed the app, the session will close.
	 * </p>
	 */
		...
		request.setCallback(new Request.Callback() {
			...
			public void onCompleted(Response response) {
				...
				if (permissionsPair != null) {
					...
					synchronized (lock) {
						tokenInfo = AccessToken.createFromTokenWithRefreshedPermissions(
								tokenInfo,
								permissionsPair.getGrantedPermissions(),
								permissionsPair.getDeclinedPermissions());
						postStateChange(state,
								SessionState.OPENED_TOKEN_UPDATED, null);
					}
				}
			}
		});
	...
	/**
	 * Internal helper class that is used to hold two different permission lists
	 * (granted and declined)
	 */
	static class PermissionsPair {
		List<String> grantedPermissions;
		List<String> declinedPermissions;

		public PermissionsPair(List<String> grantedPermissions,
				List<String> declinedPermissions) {
	...
	/**
	 * This parses a server response to a call to me/permissions. It will return
	 * the list of granted permissions. It will optionally update a session with
	 * the requested permissions. It also handles the distinction between 1.0
	 * and 2.0 calls to the endpoint.
	 *
	 * @param response
	 *            The server response
	...
	static PermissionsPair handlePermissionResponse(Response response) {
		...
		GraphMultiResult result = response
	 ...
	 * requestNewPermissions should forward the resulting onActivityResult call
	 * here to update the Session state based on the contents of the resultCode
	 * and data.
	 *
	...
	public final boolean onActivityResult(Activity currentActivity,
		...
		if (data != null) {
			...
			if (result != null) {
				...
				authorizationClient.onActivityResult(requestCode, resultCode,
						data);
				return true;
			}
		} else if (resultCode == Activity.RESULT_CANCELED) {
			exception = new FacebookOperationCanceledException(
		...
		if (exception == null) {
			exception = new FacebookException(
					"Unexpected call to Session.onActivityResult");
		}
	...
	/**
	 * Adds a callback that will be called when the state of this Session
	 * changes.
	 *
	 * @param callback
	 *            the callback
	...
	void extendTokenCompleted(Bundle bundle) {
		synchronized (this.lock) {
			...
			this.tokenInfo = AccessToken.createFromRefresh(this.tokenInfo,
					bundle);
			if (this.tokenCachingStrategy != null) {
	...
	private Object writeReplace() {
		return new SerializationProxyV1(applicationId, state, tokenInfo,
				lastAttemptedTokenExtendDate, false,
				pendingAuthorizationRequest);
	}
	...
	private void readObject(ObjectInputStream stream)
		...
		throw new InvalidObjectException(
				"Cannot readObject, serialization proxy required");
	}

	/**
	 * Save the Session object into the supplied Bundle. This method is intended
	 * to be called from an Activity or Fragment's onSaveInstanceState method in
	 * order to preserve Sessions across Activity lifecycle events.
	 *
	 * @param session
	 *            the Session to save
	 * @param bundle
	...
	public static final void saveSession(Session session, Bundle bundle) {
		if (bundle != null && session != null
				&& !bundle.containsKey(SESSION_BUNDLE_SAVE_KEY)) {
			...
			bundle.putByteArray(SESSION_BUNDLE_SAVE_KEY,
	 ...
	 * be called from an Activity or Fragment's onCreate method when a Session
	 * has previously been saved into a Bundle via saveState to preserve a
	 * Session across Activity lifecycle events.
	 *
	 * @param context
	 *            the Activity or Service creating the Session, must not be null
	 * @param cachingStrategy
	 *            the TokenCachingStrategy to use to load and store the token.
	 *            If this is null, a default token cachingStrategy that stores
	 *            data in SharedPreferences will be used
	 * @param callback
	...
	public static final Session restoreSession(Context context,
			TokenCachingStrategy cachingStrategy, StatusCallback callback,
			Bundle bundle) {
		if (bundle == null) {
			return null;
		}
		...
		if (data != null) {
			...
			try {
				Session session = (Session) (new ObjectInputStream(is))
						.readObject();
				initializeStaticContext(context);
				...
				session.authorizationBundle = bundle
						.getBundle(AUTH_BUNDLE_SAVE_KEY);
				return session;
	...
	/**
	 * If a cached token is available, creates and opens the session and makes
	 * it active without any user interaction, otherwise this does nothing.
	 *
	 * @param context
	 ...
	 *            regarding Session state changes. May be null.
	 * @return The new Session or null if one could not be created
	 */
	public static Session openActiveSession(Activity activity,
			boolean allowLoginUI, StatusCallback callback) {
		return openActiveSession(activity, allowLoginUI, new OpenRequest(
	 ...
	 * If allowLoginUI is true, this will create a new Session, make it active,
	 * and open it. If the default token cache is not available, then this will
	 * request the permissions provided (and basic permissions of no permissions
	 * are provided). If the default token cache is available and cached tokens
	 ...
	 *            not require user interaction
	 * @param permissions
	 *            The permissions to request for this Session
	 * @param callback
	 *            The {@link StatusCallback SessionStatusCallback} to notify
	 *            regarding Session state changes. May be null.
	 * @return The new Session or null if one could not be created
	 */
	public static Session openActiveSession(Activity activity,
			boolean allowLoginUI, List<String> permissions,
			StatusCallback callback) {
		return openActiveSession(activity, allowLoginUI, new OpenRequest(
				activity).setCallback(callback).setPermissions(permissions));
	}
	...
	public static Session openActiveSession(Context context, Fragment fragment,
			boolean allowLoginUI, StatusCallback callback) {
		return openActiveSession(context, allowLoginUI, new OpenRequest(
	...
	public static Session openActiveSession(Context context, Fragment fragment,
			...
			StatusCallback callback) {
		return openActiveSession(context, allowLoginUI, new OpenRequest(
				fragment).setCallback(callback).setPermissions(permissions));
	}
	 ...
	 * Session/TokenCachingStrategy-based session-management system. A primary
	 * example would be an application which previously did not use the Facebook
	 * SDK for Android and implemented its own session-management scheme, but
	 * wishes to implement an upgrade path for existing users so they do not
	 * need to log in again when upgrading to a version of the app that uses the
	 * SDK. In general, this method will be called only once, when the app
	 * detects that it has been upgraded -- after that, the usual Session
	 * lifecycle methods should be used to manage the session and its associated
	 * token.
	 * <p/>
	 * No validation is done that the token, token source, or permissions are
	 * actually valid. It is the caller's responsibility to ensure that these
	...
	private static Session openActiveSession(Context context,
			boolean allowLoginUI, OpenRequest openRequest) {
	...
	void authorize(AuthorizationRequest request) {
		...
		pendingAuthorizationRequest.loggingExtras.put(
				AuthorizationClient.EVENT_EXTRAS_TRY_LOGIN_ACTIVITY,
				started ? AppEventsConstants.EVENT_PARAM_VALUE_YES
						: AppEventsConstants.EVENT_PARAM_VALUE_NO);

		if (!started && request.isLegacy) {
			pendingAuthorizationRequest.loggingExtras.put(
					AuthorizationClient.EVENT_EXTRAS_TRY_LEGACY,
		...
		if (!started) {
			synchronized (this.lock) {
				...
				default:
					...
					logAuthorizationComplete(
							AuthorizationClient.Result.Code.ERROR, null,
							exception);
	...
	private void handleAuthorizationResult(int resultCode,
		...
		if (resultCode == Activity.RESULT_OK) {
			if (result.code == AuthorizationClient.Result.Code.SUCCESS) {
				...
				exception = new FacebookAuthorizationException(
						result.errorMessage);
			}
		} else if (resultCode == Activity.RESULT_CANCELED) {
			exception = new FacebookOperationCanceledException(
	...
	private void logAuthorizationStart() {
		Bundle bundle = AuthorizationClient
				.newAuthorizationLoggingBundle(pendingAuthorizationRequest
						.getAuthId());
		bundle.putLong(AuthorizationClient.EVENT_PARAM_TIMESTAMP,
		...
		try {
			...
			extras.put(AuthorizationClient.EVENT_EXTRAS_PERMISSIONS, TextUtils
					.join(",", pendingAuthorizationRequest.permissions));
			extras.put(AuthorizationClient.EVENT_EXTRAS_DEFAULT_AUDIENCE,
					pendingAuthorizationRequest.defaultAudience.toString());
			bundle.putString(AuthorizationClient.EVENT_PARAM_EXTRAS,
					extras.toString());
		} catch (JSONException e) {
		}

		AppEventsLogger logger = getAppEventsLogger();
		logger.logSdkEvent(AuthorizationClient.EVENT_NAME_LOGIN_START, null,
				bundle);
	}

	private void logAuthorizationComplete(
			AuthorizationClient.Result.Code result,
			Map<String, String> resultExtras, Exception exception) {
		Bundle bundle = null;
		if (pendingAuthorizationRequest == null) {
			...
			bundle.putString(
					AuthorizationClient.EVENT_PARAM_ERROR_MESSAGE,
			...
			if (resultExtras != null) {
				...
				try {
					for (Map.Entry<String, String> entry : resultExtras
							.entrySet()) {
		...
		AppEventsLogger logger = getAppEventsLogger();
		logger.logSdkEvent(AuthorizationClient.EVENT_NAME_LOGIN_COMPLETE, null,
				bundle);
	}
	...
	private boolean tryLoginActivity(AuthorizationRequest request) {
		...
		try {
			request.getStartActivityDelegate().startActivityForResult(intent,
	...
	private boolean resolveIntent(Intent intent) {
		ResolveInfo resolveInfo = getStaticContext().getPackageManager()
	...
	void finishAuthOrReauth(AccessToken newToken, Exception exception) {
		...
		synchronized (this.lock) {
			switch (this.state) {
			case OPENING:
				// This means we are authorizing for the first time in this
				// Session.
			...
			case CLOSED:
			case CLOSED_LOGIN_FAILED:
				Log.d(TAG, "Unexpected call to finishAuthOrReauth in state "
						+ this.state);
				break;
			}
		}
	...
	private void finishReauthorization(final AccessToken newToken,
			Exception exception) {
		final SessionState oldState = state;
	...
	void postStateChange(final SessionState oldState,
			final SessionState newState, final Exception exception) {
		// When we request new permissions, we stay in
		// SessionState.OPENED_TOKEN_UPDATED,
		// but we still want notifications of the state change since permissions
		// are
		// different now.
		...
		// ordering.
		// Otherwise these callbacks could have been added to the queue before
		// the SessionTracker
	...
	static void postActiveSessionAction(String action) {
		...
		LocalBroadcastManager.getInstance(getStaticContext()).sendBroadcast(
				intent);
	}

	private static void runWithHandlerOrExecutor(Handler handler,
			Runnable runnable) {
		if (handler != null) {
	...
	boolean shouldExtendAccessToken() {
		...
		if (state.isOpened()
				&& tokenInfo.getSource().canExtendToken()
	...
	private AppEventsLogger getAppEventsLogger() {
		synchronized (lock) {
			if (appEventsLogger == null) {
				appEventsLogger = AppEventsLogger.newLogger(staticContext,
						applicationId);
			}
	...
	class TokenRefreshRequest implements ServiceConnection {
		...
		public void bind() {
			Intent intent = NativeProtocol
					.createTokenRefreshIntent(getStaticContext());
			if (intent != null
					&& staticContext.bindService(intent, this,
		...
		private void refreshToken() {
			...
			requestData.putString(AccessToken.ACCESS_TOKEN_KEY, getTokenInfo()
					.getToken());
	...
	static class TokenRefreshRequestHandler extends Handler {
		...
		TokenRefreshRequestHandler(Session session,
		...
		public void handleMessage(Message msg) {
			String token = msg.getData()
					.getString(AccessToken.ACCESS_TOKEN_KEY);
	...
	public static final class Builder {
		 ...
		 * @return the Builder instance
		 */
		public Builder setTokenCachingStrategy(
				final TokenCachingStrategy tokenCachingStrategy) {
			this.tokenCachingStrategy = tokenCachingStrategy;
			return this;
		}
	...
	private void autoPublishAsync() {
		...
		synchronized (this) {
			if (autoPublishAsyncTask == null
					&& Settings.getShouldAutoPublishInstall()) {
				...
				if (applicationId != null) {
					asyncTask = autoPublishAsyncTask = new AutoPublishAsyncTask(
							applicationId, staticContext);
				}
			}
		}
	...
	/**
	 * Base class for authorization requests {@link OpenRequest} and
	 * {@link NewPermissionsRequest}.
	 */
	public static class AuthorizationRequest implements Serializable {
		...
		private AuthorizationRequest(SessionLoginBehavior loginBehavior,
				...
				String defaultAudience, boolean isLegacy, String applicationId,
		...
		AuthorizationRequest setDefaultAudience(
		...
		AuthorizationClient.AuthorizationRequest getAuthorizationClientRequest() {
			AuthorizationClient.StartActivityDelegate delegate = new AuthorizationClient.StartActivityDelegate() {
				...
				public void startActivityForResult(Intent intent,
						int requestCode) {
					startActivityDelegate.startActivityForResult(intent,
							requestCode);
				}
			...
			return new AuthorizationClient.AuthorizationRequest(loginBehavior,
					requestCode, isLegacy, permissions, defaultAudience,
					applicationId, validateSameFbidAsToken, delegate, authId);
		}

		// package private so subclasses can use it
		Object writeReplace() {
			return new AuthRequestSerializationProxyV1(loginBehavior,
					requestCode, permissions, defaultAudience.name(), isLegacy,
					applicationId, validateSameFbidAsToken);
		}

		// have a readObject that throws to prevent spoofing; must be private so
		// serializer will call it (will be
		...
		Serializable {
			...
			private AuthRequestSerializationProxyV1(
					SessionLoginBehavior loginBehavior, int requestCode,
					List<String> permissions, String defaultAudience,
					boolean isLegacy, String applicationId,
					String validateSameFbidAsToken) {
				this.loginBehavior = loginBehavior;
				this.requestCode = requestCode;
				this.permissions = permissions;
			...
			private Object readResolve() {
				return new AuthorizationRequest(loginBehavior, requestCode,
						permissions, defaultAudience, isLegacy, applicationId,
	...
	public static final class OpenRequest extends AuthorizationRequest {
		 ...
		 */
		@Override
		public final OpenRequest setLoginBehavior(
		 ...
		 *
		 * @param permissions
		 *            the permissions to request during the authentication flow.
		 * @return the OpenRequest object to allow for chaining
		 */
		@Override
		 ...
		 * <p/>
		 * This is only used during Native login using a sufficiently recent
		 * facebook app.
		 *
		 * @param defaultAudience
		 *            A SessionDefaultAudience representing the default audience
		 *            setting to request.
		 * @return the OpenRequest object to allow for chaining
		 */
		@Override
		public final OpenRequest setDefaultAudience(
				SessionDefaultAudience defaultAudience) {
	...
	AuthorizationRequest {
		...
		/**
		 * Sets the StatusCallback for the NewPermissionsRequest. Note that once
		 * the request is made, this callback will be added to the session, and
		 * will receive all future state changes on the session.
		 *
		 * @param statusCallback
		 *            The {@link StatusCallback SessionStatusCallback} to notify
		 *            regarding Session state changes.
		 ...
		 */
		@Override
		public final NewPermissionsRequest setCallback(
		 ...
		 */
		@Override
		public final NewPermissionsRequest setLoginBehavior(
				SessionLoginBehavior loginBehavior) {
			super.setLoginBehavior(loginBehavior);
			return this;
		}
