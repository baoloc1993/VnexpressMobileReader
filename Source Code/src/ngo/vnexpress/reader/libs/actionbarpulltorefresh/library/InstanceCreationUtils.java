...
import ngo.vnexpress.reader.libs.actionbarpulltorefresh.library.viewdelegates.AbsListViewDelegate;
import ngo.vnexpress.reader.libs.actionbarpulltorefresh.library.viewdelegates.ScrollYDelegate;
import ngo.vnexpress.reader.libs.actionbarpulltorefresh.library.viewdelegates.ViewDelegate;
import ngo.vnexpress.reader.libs.actionbarpulltorefresh.library.viewdelegates.WebViewDelegate;
...
class InstanceCreationUtils {
	...
	static {
		...
		addBuiltinDelegates(AbsListViewDelegate.SUPPORTED_VIEW_CLASSES,
				AbsListViewDelegate.class);
		addBuiltinDelegates(ScrollYDelegate.SUPPORTED_VIEW_CLASSES,
				ScrollYDelegate.class);
		addBuiltinDelegates(WebViewDelegate.SUPPORTED_VIEW_CLASSES,
				WebViewDelegate.class);
	}

	private static void addBuiltinDelegates(Class<?>[] supportedViews,
			Class<?> delegateClass) {
	...
	static <T> T instantiateViewDelegate(Context context, String className) {
		try {
			...
			return newInstance(context, clazz,
					VIEW_DELEGATE_CONSTRUCTOR_SIGNATURE);
		} catch (Exception e) {
	...
	static <T> T instantiateTransformer(Context context, String className) {
		try {
			Class<?> clazz = context.getClassLoader().loadClass(className);
			return newInstance(context, clazz,
					TRANSFORMER_CONSTRUCTOR_SIGNATURE);
		} catch (Exception e) {
			Log.w(LOG_TAG, "Cannot instantiate class: " + className, e);
		}
		return null;
