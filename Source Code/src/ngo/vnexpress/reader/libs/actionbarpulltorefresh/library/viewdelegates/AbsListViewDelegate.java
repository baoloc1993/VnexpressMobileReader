...
package ngo.vnexpress.reader.libs.actionbarpulltorefresh.library.viewdelegates;
...
public class AbsListViewDelegate implements ViewDelegate {
	...
	public boolean isReadyForPull(View view, final float x, final float y) {
		...
		if (ready && absListView.isFastScrollEnabled()
				&& isFastScrollAlwaysVisible(absListView)) {
	...
	int getVerticalScrollbarPosition(AbsListView absListView) {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB ? CompatV11
				.getVerticalScrollbarPosition(absListView) : Compat
				.getVerticalScrollbarPosition(absListView);
	}

	boolean isFastScrollAlwaysVisible(AbsListView absListView) {
		return Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB ? CompatV11
				.isFastScrollAlwaysVisible(absListView) : Compat
				.isFastScrollAlwaysVisible(absListView);
	}
