...
public class ActionBarPullToRefresh {
	...
	public static final class SetupWizard {
		...
		public SetupWizard useViewDelegate(Class<?> viewClass,
		...
		public void setup(PullToRefreshLayout pullToRefreshLayout) {
			PullToRefreshAttacher attacher = pullToRefreshLayout
					.createPullToRefreshAttacher(mActivity, mOptions);
			...
			if (mViewDelegates != null) {
				final Set<Map.Entry<Class<?>, ViewDelegate>> entries = mViewDelegates
						.entrySet();
		...
		private static void insertLayoutIntoViewGroup(ViewGroup viewGroup,
			...
			// Move all children to PullToRefreshLayout. This code looks a bit
			// silly but the child
			// indices change every time we remove a View (so we can't just
			// iterate through)
